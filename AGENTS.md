...
为了确保研发报告的**数据准确性**和**可追溯性**，AI 报告生成必须遵循以下“物理隔离”原则：

### 1. 信息分类与处理通道
*   **类型 A：事实值 (Fact)** - **硬绑定**。
    *   来源：数据库记录的主键 + 字段值。
    *   处理：代码直接渲染进模板占位符 `{{F:key}}`。**LLM 严禁修改此类数据。**
*   **类型 B：派生结论 (Derived Fact)** - **代码计算**。
    *   来源：对数值序列做单调性/极值/阈值判断。
    *   处理：产出结构化标签填入 `{{B:key}}`。LLM 仅负责将其“翻译成人话”。
*   **类型 C：软性文字 (Prose)** - **模型生成**。
    *   来源：背景介绍、机理描述、讨论展望。
    *   处理：LLM 根据上下文生成，填入 `{{P:key}}`。

### 2. 模板引擎要求
*   禁止使用 LLM 全文自由生成。
*   必须采用“骨架文本 + 槽位占位符”模式。
*   填充顺序：先填 A+B，再逐段调 LLM 填 C，最后代码拼回完整文档。

### 3. 数值校验层 (生成后自动跑)
每次生成后，必须运行纯代码校验脚本，红线项包括：
*   **数值一致性**：报告中所有数字必须与数据库 Fact 对照一致。
*   **单位/量纲**：检查 kg/g、℃、% 等是否正确。
*   **术语合法性**：专业词汇必须在白名单内。

### 4. 可追溯性
*   每个 Fact 槽位在填充时必须附带溯源标记（如 `record_id`）。
*   导出的报告应支持点击数据点反查原始电子实验记录 (ELN)。
>>>>>>> c867997f (feat: 完成严谨版 AI 报告生成方案核心逻辑与 API 接口)

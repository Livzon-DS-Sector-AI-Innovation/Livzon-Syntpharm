# 67 — Flip `@typescript-eslint/no-explicit-any` rule to error
...
- [x] `@typescript-eslint/no-explicit-any` rule changed from "warn" to "error" in `eslint.config.mjs`
- [x] `eslint-disable` comments from ticket 59 retained for `apiFetch` and server API functions
- [x] `pnpm lint` passes with 0 errors and 0 warnings (except for the eslint-disable cases)
- [x] `tsc --noEmit` passes with no errors
- [x] CI will now fail on any new `any` usage

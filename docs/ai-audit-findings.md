...
**Changed files (22):**
- `backend/app/main.py` — removes OCR init from lifespan
- `backend/app/modules/registration/dossier_writer/ai_fill_service.py` — asyncio.to_thread wrappers, split_id in response, PageInsertDetail, auto-insert logic
- `backend/app/modules/registration/dossier_writer/asset_processor.py` — OCR subprocess migration
- `backend/app/modules/registration/dossier_writer/asset_text_extractor.py` — OCR subprocess migration, removes _ensure_ocr_service
- `backend/app/modules/registration/dossier_writer/field_fill_service.py` — asyncio.to_thread wrapper
- `backend/app/modules/registration/dossier_writer/schemas.py` — new PageInsertDetail, split_id field
- `backend/app/modules/registration/service/validation_audit.py` — asyncio.to_thread wrapper
- `backend/app/modules/safety/ai_hazard_identification/orchestrator.py` — asyncio.to_thread wrapper
- `backend/app/modules/safety/knowledge/document_loader.py` — asyncio.to_thread wrapper
- `backend/app/modules/safety/service/attachment.py` — asyncio.to_thread wrapper
- `backend/app/modules/safety/service/safety.py` — asyncio.to_thread wrapper
- `backend/app/platform/integrations/ai/document_parser.py` — OCR subprocess migration
- `backend/app/shared/ocr_service.py` — new _run_ocr_in_subprocess, count_pdf_pages, global lock
- `backend/app/shared/ocr_worker.py` — new file, standalone OCR worker subprocess
- `backend/pyproject.toml` — adds psutil + types-psutil dependencies
- `backend/scripts/seed/seed_s6_ai_config.py` — new seed script for chapter 3.2.S.6
- `backend/uv.lock` — registry URLs switched from mirrors.aliyun.com to pypi.org
- `docker-compose.dev.yml` — read-only backend mount, PYTHONDONTWRITEBYTECODE
- `frontend/src/actions/dossier-writer.ts` — AbortSignal.timeout, SplitConfirmResult type extension
- `frontend/src/components/registration/AiFillPanel.tsx` — batch insert UI, selectedSplits state
- `frontend/src/types/dossier-writer.ts` — split_id field added to PageSplitInfo

**Affected categories:** 1, 2, 3, 5, 6, 7, 9, 10, 13, 15
...
**Analysis:**
- `backend/scripts/seed/seed_s6_ai_config.py` is correctly placed in `scripts/seed/` per the scripts directory table
- No files in deprecated `app/models/`, `app/schemas/`, or `app/integrations/` directories
- No `.docx`, `.xlsx`, or PDF files in `backend/` root
- `backend/app/shared/ocr_worker.py` correctly placed in `app/shared/` (global layer)
- Frontend files in correct locations (`components/registration/`, `actions/`, `types/`)
...
**Analysis:**
- All cross-module imports go through `app.shared.*` or `app.platform.*` (global layers) — allowed
- `asset_processor.py`, `asset_text_extractor.py` import from `app.shared.ocr_service` — OK
- `document_parser.py` is in `app.platform.integrations.ai` — OK
- `validation_audit.py`, `orchestrator.py`, `document_loader.py`, `attachment.py`, `safety.py` import from `app.platform.integrations.ai.document_parser` — OK (platform is global)
- No cross-module imports between business modules
- No circular dependencies introduced
- No new module directories created
...
**Analysis:**
- `schemas.py` adds `PageInsertDetail` Pydantic model and `split_id` field to `PageSplitItem` — these are response schemas, not ORM models. No migration needed.
...
**Analysis:**
- `docker-compose.dev.yml`: Adds `PYTHONDONTWRITEBYTECODE=1` — standard Python env var, no issue.
- `pyproject.toml`: Adds `psutil>=5.9.0` and `types-psutil>=5.9.0` — legitimate dependencies for memory monitoring.
- `main.py`: Removes `init_ocr` from lifespan — OCR is now lazy-loaded in subprocess. Consistent with the new architecture.
- All logging uses `logging.getLogger(__name__)` — correct pattern.
...
**Analysis:**
- No direct LLM client construction — uses `llm_client` singleton throughout.
- No direct `import paddleocr` in module code — only in `app/shared/ocr_service.py` and `app/shared/ocr_worker.py` (shared layer, allowed).
- No direct `import apscheduler`.
- No `asyncio.create_task()` in business logic — the PR *removes* `asyncio.create_task(init_ocr)` from `main.py`.
- All OCR calls wrapped in `asyncio.to_thread()` to avoid blocking the event loop — correct pattern.
- Subprocess isolation: `start_new_session=True` + `os.killpg()` for process group kill — correct.
- Memory gate: `psutil.virtual_memory()` check before starting subprocess — correct.
- Timeout: `max(provided, 120 + 60 * pages)` — reasonable formula.
- Global lock: `threading.Lock()` serializes OCR calls — correct.
...
**Analysis:**
- `AiFillPanel.tsx` is in `frontend/src/components/registration/` — correct location.
- Component uses standard React hooks and antd components.
- No barrel file violations.
...
**Analysis:**
- `dossier-writer.ts` actions file imports base types from `@/types/generated/schema` — correct.
- However, the backend schema `SplitConfirmData` was updated to include `details: list[PageInsertDetail]`, but the frontend generated types were NOT regenerated. The generated `SplitConfirmData` still lacks the `details` field.
- Similarly, `PageSplitItem` in generated types lacks `split_id` which was added to the backend schema.
...
| Files inspected | 2 |
| Files not inspected | 0 |
| Rules evaluated | 8 (all Q1-D8) |
| Rules not evaluated | 0 |
| Confirmed findings | 0 |
| Uncertain findings | 0 |

**Analysis:**
- `docker-compose.dev.yml`: Adds `./backend:/app/backend:ro` (read-only mount for backend source) and `PYTHONDONTWRITEBYTECODE=1`. The read-only mount is intentional per PR description ("后端代码挂载只读化"). Temp files are written to `/tmp`, not to mounted dirs, so OCR subprocess is unaffected.
- `docker-compose.dev.yml` correctly specifies `target: dev` for frontend.
- No hardcoded localhost/127.0.0.1 URLs.
...
**Analysis:**
- `seed_s6_ai_config.py`: Uses `asyncpg` with parameterized queries (`$1`, `$2`, etc.) — correct.
- `validation_audit.py`: Uses SQLAlchemy ORM — correct.
- `ai_fill_service.py`: Uses SQLAlchemy ORM (`select`, `and_`, column comparisons) — correct.
- `safety.py`: Uses SQLAlchemy ORM — correct.
- No f-strings, string concatenation, or `.format()` in SQL construction.

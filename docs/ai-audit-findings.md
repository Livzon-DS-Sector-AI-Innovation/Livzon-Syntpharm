...
- [ ] `frontend/src/lib/api/server/ai.ts:4` — AGENTS.md apiFetch一致性/Q9 — **Regression from origin/main.** Removed `import { getApiBaseUrl } from '@/lib/api/server/base'` and added duplicate `const API_BASE = process.env.API_BASE_URL || 'http://backend:8000'`. On origin/main, `ai.ts` correctly imported `getApiBaseUrl()` from `base.ts`. This PR reverses that fix. Additionally, `generateExamQuestions` (line 10) and `parseExperimentRecord` (line 30) now use raw `fetch()` with `API_BASE` instead of `apiFetch()`, **losing auth header forwarding** — `apiFetch()` calls `getAuthHeaders()` which reads `auth_token` cookie, but raw `fetch()` does not. — severity: **blocking**

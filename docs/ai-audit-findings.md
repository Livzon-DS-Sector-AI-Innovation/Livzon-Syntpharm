...
### PR #26: Ruanjiaheng — apiFetch consistency enforcement (head: ruanjiaheng, base: main, date: 2026-08-11)

Files changed: 49 across categories 1, 2, 9, 10, 11 (core: consolidate getApiBaseUrl into base.ts, delete http-client.ts/http-server.ts, refactor all lib/api/client/* and lib/api/server/* to canonical utilities, replace raw fetch() in pages/components with apiGet/apiFetch)

#### Category 1: Repository layout

| Files inspected | 2 deleted files |
| Rules evaluated | 9 |
| Confirmed findings | 0 |
| Uncertain findings | 0 |

- `frontend/src/lib/http-client.ts` — **DELETED** (82 lines). Consolidated into `lib/api/client.ts`. ✓
- `frontend/src/lib/http-server.ts` — **DELETED** (15 lines). Consolidated into `lib/api/server/base.ts`. ✓
- `layout.tsx` — Removed `import '@/lib/http-server'` (dead code after deletion). ✓
- No new files created — `client.ts` and `server/base.ts` already existed on main (modified, not created). ✓

#### Category 2: Secrets and hardcoded values

| Files inspected | 49 (all changed files) |
| Rules evaluated | 9 |
| Confirmed findings | 1 |
| Uncertain findings | 0 |
...
- [x] `frontend/src/lib/api/server/ai.ts:4` — AGENTS.md apiFetch一致性/Q9 — **Regression from origin/main.** Removed `import { getApiBaseUrl } from '@/lib/api/server/base'` and added duplicate `const API_BASE = process.env.API_BASE_URL || 'http://backend:8000'`. On origin/main, `ai.ts` correctly imported `getApiBaseUrl()` from `base.ts`. This PR reverses that fix. Additionally, `generateExamQuestions` (line 10) and `parseExperimentRecord` (line 30) now use raw `fetch()` with `API_BASE` instead of `apiFetch()`, **losing auth header forwarding** — `apiFetch()` calls `getAuthHeaders()` which reads `auth_token` cookie, but raw `fetch()` does not. — severity: **blocking**

##### Verified clean

- All other server files correctly import `getApiBaseUrl` from `base.ts`. No other duplicates. ✓
- All `API_BASE` variables in page/component files were **removed** — replaced by `apiGet()` from `@/lib/api/client`. ✓
- No new hardcoded `localhost`/`127.0.0.1`, `NEXT_PUBLIC_API_BASE_URL`, absolute paths, or API keys. ✓

#### Category 9: Frontend component boundaries

| Files inspected | 10 (5 pages, 5 components) |
| Rules evaluated | 9 |
| Confirmed findings | 1 |
| Uncertain findings | 0 |
...
- [x] `frontend/src/proxy.ts:3-4` — AGENTS.md 禁止修改的文件/Q8 — Added a 2-line documentation comment explaining why `getApiBaseUrl` cannot be imported from `base.ts` in the middleware context. proxy.ts is listed under "以下文件只有架构负责人可以修改". The actual proxy logic (lines 5-30) is unchanged — only a comment was added. — severity: **medium** — **ACCEPTED** (documentation improvement; comment-only change; proxy logic unchanged)

##### Verified clean (all positive changes)

All 9 page/component files correctly refactored from raw `fetch()` + `.data` to `apiGet()`/`fetchApi()` from `@/lib/api/client`, with `API_BASE` constants removed:
`evaluation-form/page.tsx`, `sop-catalog/page.tsx`, `trainers/page.tsx`, `projects/page.tsx`, `AnnualPlanDetailClient.tsx`, `DepartureClient.tsx`, `OnboardingPrejobClient.tsx`, `SystemSettingsClient.tsx`, `AttachmentPreview.tsx` — all ✓

- Q1-Q7, Q9: All pass (headings, `'use client'`, naming, barrel exports verified). ✓
...
| Area | Change |
|---|---|
| `http-client.ts` / `http-server.ts` | **DELETED** |
| `server/base.ts` | Added `safeApiFetch`, `apiFetchPaginated` — canonical exports ✓ |
| `client.ts` | Added `apiGet`, `apiFetchPaginated` — canonical exports ✓ |
| `procurement.ts` | Custom `apiFetch` **removed**; replaced with import from `./base` ✓ |
| `safety.ts` | Duplicate `getApiV1Url`/`buildQueryString` **removed** ✓ |
| `auth.ts` | `getApiBaseCandidates` hardcoded hosts **removed** ✓ |
| `warehouse.ts`, `energy.ts`, `quality.ts`, `deviation.ts`, `agent-skills.ts`, `instrument.ts`, `admin.ts` | All consolidated to `apiFetch`/`unwrapResponse`/`apiFetchRaw` ✓ |
| Client modules (14 files) | All consolidated to `fetchApi`/`apiGet`/`apiFetchPaginated` from `client.ts` ✓ |
...
| Files inspected | 1 (proxy.ts) |
| Rules evaluated | 6 |
| Confirmed findings | 0 |
| Uncertain findings | 0 |

- `proxy.ts` properly forwards `/api/v1/*`, no prohibited operations. ✓
- `getApiBaseUrl` inline is a documented middleware exception. ✓

#### Categories not affected

3-8, 12-14 — no backend Python, models, migrations, tests, Docker, or E2E changes.
...
- Category 2: 7 hardcoded `getApiBaseUrl` duplicates removed ✓
- Category 4: `cpv_products.py` soft-delete docstring ✓
- Category 6: `cpv_products.py` logger ✓
- Category 9: `cpv/page.tsx` h1 heading ✓
- Category 10: `safety/helpers.ts` `getApiV1Url` fixed ✓

#### Previously resolved from PR #24 — cross-check

- Category 2: `FEISHU__PLATFORM__REDIRECT_URI` removed ✓
- Category 10: `'use server'` files `export type` removal ✓

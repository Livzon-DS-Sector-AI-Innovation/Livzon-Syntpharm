...
async def get_project_by_id(db: AsyncSession, project_id: uuid.UUID) -> ResearchProject | None:
...
async def get_projects(
    ...
    if project_type is not None:
        if project_type == "":
            query = query.where(ResearchProject.project_type is None)
            count_query = count_query.where(ResearchProject.project_type is None)
        else:
    ...
    query = query.order_by(ResearchProject.updated_at.desc()).offset((page - 1) * page_size).limit(page_size)
...
async def get_workflow_by_id(db: AsyncSession, workflow_id: uuid.UUID) -> PilotWorkflow | None:
    result = await db.execute(
...
async def get_workflows(
    ...
    query = query.order_by(PilotWorkflow.created_at.desc()).offset((page - 1) * page_size).limit(page_size)
    result = await db.execute(query)
    return result.scalars().all(), total  # type: ignore[return-value]
...
async def get_workflow_steps(db: AsyncSession, workflow_id: uuid.UUID) -> list[PilotWorkflowStep]:
    result = await db.execute(
...
async def get_workflow_step_by_id(db: AsyncSession, step_id: uuid.UUID) -> PilotWorkflowStep | None:
    result = await db.execute(
...
async def get_rd_project_by_id(db: AsyncSession, project_id: uuid.UUID) -> RdProject | None:
    """根据 ID 获取 RdProject"""
    result = await db.execute(
...
async def create_rd_project(db: AsyncSession, data: dict[str, Any]) -> RdProject:
    """创建 RdProject"""
...
async def update_rd_project(db: AsyncSession, project: RdProject, data: dict[str, Any]) -> RdProject:
    """更新 RdProject"""
    for key, value in data.items():
...
async def delete_rd_project(db: AsyncSession, project: RdProject) -> None:
    """删除 RdProject（软删除）"""
    project.is_deleted = True
    await db.flush()
...
async def get_milestones_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdMilestone]:
    """获取项目的所有里程碑"""
    result = await db.execute(
        select(RdMilestone)
        .where(
...
async def get_milestone_by_id(db: AsyncSession, milestone_id: uuid.UUID) -> RdMilestone | None:
    """根据 ID 获取里程碑"""
    result = await db.execute(
...
async def create_milestone(db: AsyncSession, data: dict[str, Any]) -> RdMilestone:
    """创建里程碑"""
...
async def update_milestone(db: AsyncSession, milestone: RdMilestone, data: dict[str, Any]) -> RdMilestone:
    """更新里程碑"""
    for key, value in data.items():
        if value is not None:
...
async def get_stages_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdStageRecord]:
    """获取项目的所有阶段记录"""
    result = await db.execute(
...
async def get_stage_by_id(db: AsyncSession, record_id: uuid.UUID) -> RdStageRecord | None:
    """根据 ID 获取阶段记录"""
    result = await db.execute(
...
async def create_stage_record(db: AsyncSession, data: dict[str, Any]) -> RdStageRecord:
    """创建阶段记录"""
...
async def update_stage_record(db: AsyncSession, record: RdStageRecord, data: dict[str, Any]) -> RdStageRecord:
    """更新阶段记录"""
    for key, value in data.items():
        if value is not None:
...
async def get_tracks_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdResearchTrack]:
    """获取项目的所有研究项"""
    result = await db.execute(
...
async def get_track_by_id(db: AsyncSession, track_id: uuid.UUID) -> RdResearchTrack | None:
    """根据 ID 获取研究项"""
    result = await db.execute(
...
async def create_research_track(db: AsyncSession, data: dict[str, Any]) -> RdResearchTrack:
    """创建研究项"""
...
async def update_research_track(db: AsyncSession, track: RdResearchTrack, data: dict[str, Any]) -> RdResearchTrack:
    """更新研究项"""
    for key, value in data.items():
        if value is not None:
...
async def get_findings_by_track(db: AsyncSession, track_id: uuid.UUID) -> list[RdResearchFinding]:
    """获取研究项的所有发现"""
    result = await db.execute(
...
async def get_finding_by_id(db: AsyncSession, finding_id: uuid.UUID) -> RdResearchFinding | None:
    """根据 ID 获取研究发现"""
    result = await db.execute(
...
async def create_research_finding(db: AsyncSession, data: dict[str, Any]) -> RdResearchFinding:
    """创建研究发现"""
...
async def get_pilot_studies_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdPilotStudy]:
    """获取项目的所有中试研究记录"""
    result = await db.execute(
        select(RdPilotStudy)
        .where(
...
async def get_pilot_study_by_id(db: AsyncSession, study_id: uuid.UUID) -> RdPilotStudy | None:
    """根据 ID 获取中试研究记录"""
    result = await db.execute(
...
async def create_pilot_study(db: AsyncSession, data: dict[str, Any]) -> RdPilotStudy:
    """创建中试研究记录"""
...
async def update_pilot_study(db: AsyncSession, study: RdPilotStudy, data: dict[str, Any]) -> RdPilotStudy:
    """更新中试研究记录"""
    for key, value in data.items():
        if value is not None:
...
async def get_validations_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdProcessValidation]:
    """获取项目的所有工艺验证记录"""
    result = await db.execute(
...
async def get_validation_by_id(db: AsyncSession, validation_id: uuid.UUID) -> RdProcessValidation | None:
    """根据 ID 获取工艺验证记录"""
    result = await db.execute(
...
async def create_validation(db: AsyncSession, data: dict[str, Any]) -> RdProcessValidation:
    """创建工艺验证记录"""
...
async def get_filings_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[RdRegistrationFiling]:
    """获取项目的所有申报资料记录"""
    result = await db.execute(
...
async def get_filing_by_id(db: AsyncSession, filing_id: uuid.UUID) -> RdRegistrationFiling | None:
    """根据 ID 获取申报资料记录"""
    result = await db.execute(
...
async def create_filing(db: AsyncSession, data: dict[str, Any]) -> RdRegistrationFiling:
    """创建申报资料记录"""
...
async def update_filing(db: AsyncSession, filing: RdRegistrationFiling, data: dict[str, Any]) -> RdRegistrationFiling:
    """更新申报资料记录"""
    for key, value in data.items():
        if value is not None:
...
async def create_rd_stage_deliverable(db: AsyncSession, data: dict[str, Any]) -> RdStageDeliverable:
    """创建阶段交付物"""
...
async def update_rd_stage_deliverable(
    db: AsyncSession, deliverable: RdStageDeliverable, data: dict[str, Any]
) -> RdStageDeliverable:
    """更新阶段交付物"""
    for key, value in data.items():
...
async def delete_rd_stage_deliverable(db: AsyncSession, deliverable: RdStageDeliverable) -> None:
    """删除阶段交付物"""
...
async def get_experiment_logs_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[Any]:
    """获取项目的所有实验记录"""
...
async def get_experiment_log_by_id(db: AsyncSession, log_id: uuid.UUID) -> Any:
    """根据 ID 获取实验记录"""
    from app.modules.research.models import RdExperimentLog
...
async def create_experiment_log(db: AsyncSession, data: dict[str, Any]) -> Any:
    """创建实验记录"""
    from app.modules.research.models import RdExperimentLog
...
async def update_experiment_log(db: AsyncSession, log, data: dict[str, Any]) -> Any:  # type: ignore[no-untyped-def]
    """更新实验记录"""
    for key, value in data.items():
        if value is not None:
...
async def get_reports_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[Any]:
    """获取项目的所有研发报告"""
...
async def get_report_by_id(db: AsyncSession, report_id: uuid.UUID) -> Any:
    """根据 ID 获取研发报告"""
    from app.modules.research.models import RdReport
...
async def create_report(db: AsyncSession, data: dict[str, Any]) -> Any:
    """创建研发报告"""
    from app.modules.research.models import RdReport
...
async def update_report(db: AsyncSession, report, data: dict[str, Any]) -> Any:  # type: ignore[no-untyped-def]
    """更新研发报告"""
    for key, value in data.items():
        if value is not None:
...
async def get_initiations_by_project(db: AsyncSession, project_id: uuid.UUID) -> list[Any]:
    """获取项目的所有立项申请"""
...
async def get_initiation_by_id(db: AsyncSession, initiation_id: uuid.UUID) -> Any:
    """根据 ID 获取立项申请"""
    from app.modules.research.models import RdInitiation
...
async def create_initiation(db: AsyncSession, data: dict[str, Any]) -> Any:
    """创建立项申请"""
    from app.modules.research.models import RdInitiation
...
async def update_initiation(db: AsyncSession, initiation, data: dict[str, Any]) -> Any:  # type: ignore[no-untyped-def]
    """更新立项申请"""
    for key, value in data.items():
        if value is not None:
...
async def get_conclusion_versions(db: AsyncSession, track_id: uuid.UUID) -> list[Any]:
    """获取研究项的结论版本历史"""
...
async def create_conclusion_version(db: AsyncSession, data: dict[str, Any]) -> Any:
    """创建结论版本"""
    from app.modules.research.models import RdTrackConclusionVersion
...
async def get_latest_conclusion_version(db: AsyncSession, track_id: uuid.UUID) -> Any:
    """获取最新结论版本"""
    from app.modules.research.models import RdTrackConclusionVersion
...
async def get_deliverable_template_by_id(db: AsyncSession, template_id: uuid.UUID) -> Any:
    """根据 ID 获取交付物模板"""
    from app.modules.research.models import RdDeliverableTemplate
...
async def create_deliverable_template(db: AsyncSession, data: dict[str, Any]) -> Any:
    """创建交付物模板"""
    from app.modules.research.models import RdDeliverableTemplate
...
async def update_deliverable_template(db: AsyncSession, template, data: dict[str, Any]) -> Any:  # type: ignore[no-untyped-def]
    """更新交付物模板"""
    for key, value in data.items():
        if value is not None:

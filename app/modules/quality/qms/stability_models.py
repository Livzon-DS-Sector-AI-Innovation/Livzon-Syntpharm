...
class StabilityStudy(BaseModel):
    ...
    # 方案编号
    study_no: Mapped[str] = mapped_column(
        String(64), nullable=False, unique=True, comment="试验方案编号"
    )
    ...
    product_category: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="产品类别"
    )

    # 批次信息
    batch_no: Mapped[str] = mapped_column(String(64), nullable=False, comment="批号")
    batch_quantity: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 6), nullable=True, comment="批量"
    )
    packaging_spec: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="包装规格"
    )

    # 试验条件
    study_type: Mapped[str] = mapped_column(
        String(32), nullable=False, comment="试验类型"
    )
    temperature: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="温度条件"
    )
    humidity: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="湿度条件"
    )

    # 试验周期
    start_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="试验开始日期"
    )
    end_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="试验结束日期"
    )
    ...
    # 取样周期
    sample_intervals: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="取样周期节点"
    )
    ...
    standard_name: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="质量标准名称"
    )
    ...
    developer_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("identity.users.id"), nullable=True, comment="研发人员ID"
    )
    developer_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="研发人员"
    )
    ...
    # 方案结论
    study_conclusion: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="试验结论"
    )
    ...
    attachments: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="附件JSON"
    )
    ...
    # 关系
    sample_nodes: Mapped[list["StabilitySampleNode"]] = relationship(
        back_populates="stability_study", lazy="selectin"
    )
    inspections: Mapped[list["StabilityInspection"]] = relationship(
        back_populates="stability_study", lazy="selectin"
    )
...
class StabilitySampleNode(BaseModel):
    ...
    stability_study_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.stability_studies.id"),
        nullable=False,
        comment="稳定性试验ID",
    )
    ...
    node_name: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="节点名称"
    )

    # 计划日期
    planned_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="计划取样日期"
    )

    # 实际日期
    actual_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="实际取样日期"
    )

    # 取样数量
    sample_quantity: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 6), nullable=True, comment="取样数量"
    )

    # 状态
    status: Mapped[str] = mapped_column(
        String(32), server_default="pending", nullable=False, comment="状态"
    )

    # 预警状态
    reminder_sent: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否已发送预警"
    )
    reminder_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="预警发送时间"
    )

    # 检验记录（不使用外键约束，保留字段用于关联）
    inspection_id: Mapped[UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="关联检验记录ID"
    )
    inspection_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="检验记录编号"
    )
    inspection_status: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="检验状态"
    )
    inspection_conclusion: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="检验结论"
    )
    ...
    # 关系
    stability_study: Mapped["StabilityStudy"] = relationship(
        back_populates="sample_nodes"
    )
...
class StabilityInspection(BaseModel):
    ...
    study_no: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="试验方案编号"
    )
    sample_node_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.stability_sample_nodes.id"),
        nullable=False,
        comment="取样节点ID",
    )
    node_month: Mapped[int] = mapped_column(Integer, nullable=False, comment="节点月数")
    ...
    specification: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="规格/包装"
    )
    ...
    inspector_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="检验员"
    )
    ...
    sample_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="样品编号"
    )
    sample_condition: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="样品状态"
    )
    ...
    # OOS
    oos_report_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="OOS报告编号"
    )
    is_oos: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否OOS"
    )
    ...
    sample_node: Mapped["StabilitySampleNode"] = relationship(
        foreign_keys=[sample_node_id]
    )
    items: Mapped[list["StabilityInspectionItem"]] = relationship(
        back_populates="stability_inspection", lazy="selectin"
    )
    approval_records: Mapped[list["StabilityApprovalRecord"]] = relationship(
        foreign_keys="StabilityApprovalRecord.inspection_id", lazy="selectin"
    )
...
class StabilityInspectionItem(BaseModel):
    ...
    stability_inspection_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.stability_inspections.id"),
        nullable=False,
        comment="稳定性检验ID",
    )
    ...
    standard_value: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="标准值/限度"
    )
    ...
    result: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="单项判定"
    )
    is_oos: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否超标"
    )
    oos_description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="超标描述"
    )

    # 趋势分析相关
    data_point: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="数据点"
    )

    # 图谱附件
    chromatogram_urls: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="图谱附件JSON"
    )
    remark: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")
    ...
    # 关系
    stability_inspection: Mapped["StabilityInspection"] = relationship(
...
class StabilityApprovalRecord(BaseModel):
    ...
    approval_level: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="审批级别"
    )
    ...
    # 关系
    stability_study: Mapped[Optional["StabilityStudy"]] = relationship(
        back_populates="approval_records"
    )
    inspection: Mapped[Optional["StabilityInspection"]] = relationship(
        back_populates="approval_records"
    )

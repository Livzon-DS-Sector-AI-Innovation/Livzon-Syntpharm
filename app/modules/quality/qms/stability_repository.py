...
class StabilityStudyRepository:
    ...
    async def get_by_study_no(self, study_no: str) -> StabilityStudy | None:
        """根据方案编号查询"""
        result = await self.session.execute(
            select(StabilityStudy).where(
                and_(
    ...
    async def generate_study_no(self) -> str:
        ...
        result = await self.session.execute(
            select(StabilityStudy.study_no)
            .where(
                and_(
...
class StabilitySampleNodeRepository:
    ...
    async def get_by_id(self, node_id: UUID) -> StabilitySampleNode | None:
        """根据ID获取节点"""
        result = await self.session.execute(
...
class StabilityInspectionRepository:
    ...
    async def get_by_id(self, inspection_id: UUID) -> StabilityInspection | None:
        """根据ID获取检验记录"""
        result = await self.session.execute(
    ...
    async def get_by_node_id(self, node_id: UUID) -> StabilityInspection | None:
        """根据节点ID获取检验记录"""
        result = await self.session.execute(
            select(StabilityInspection).where(
                and_(
    ...
    async def generate_inspection_no(self) -> str:
        ...
        result = await self.session.execute(
            select(StabilityInspection.inspection_no)
            .where(
                and_(
...
class StabilityInspectionItemRepository:
    ...
    async def get_by_id(self, item_id: UUID) -> StabilityInspectionItem | None:
        """根据ID获取明细"""
        result = await self.session.execute(

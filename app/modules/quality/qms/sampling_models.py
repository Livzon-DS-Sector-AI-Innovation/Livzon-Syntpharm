...
class SamplingOrder(BaseModel):
    ...
    order_no: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="取样单号"
    )
    source_type: Mapped[str] = mapped_column(
        String(32), nullable=False, comment="来源类型：purchase_inbound/batch_no"
    )
    source_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="关联单号"
    )
    ...
    material_category: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="物料类别"
    )
    ...
    quantity: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 6), nullable=True, comment="批量/数量"
    )
    sampling_source: Mapped[str] = mapped_column(
        String(32), nullable=False, comment="取样来源"
    )
    sampling_quantity: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 6), nullable=True, comment="取样量"
    )
    sampling_location: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="取样地点"
    )
    sampling_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="取样日期"
    )
    sampler_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("identity.users.id"), nullable=True, comment="取样人ID"
    )
    sampler_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="取样人姓名"
    )
    status: Mapped[str] = mapped_column(
        String(32), server_default="draft", nullable=False, comment="状态"
    )
    sampling_result: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="取样判定"
    )
    exception_reasons: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="异常原因"
    )
    ...
    # 关系
    items: Mapped[list["SamplingOrderItem"]] = relationship(
        back_populates="sampling_order", lazy="selectin"
    )
    approval_records: Mapped[list["SamplingApprovalRecord"]] = relationship(
        back_populates="sampling_order", lazy="selectin"
    )
...
class SamplingOrderItem(BaseModel):
    ...
    sampling_order_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.sampling_orders.id"), nullable=False
    )
    item_no: Mapped[int] = mapped_column(Integer, nullable=False, comment="项次")
    sample_no: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="样品编号"
    )
    sampling_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="取样份数"
    )
    retention_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="留样份数"
    )
    retention_location: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="留样存放位置"
    )
    sample_status: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="样品状态"
    )
    retention_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="留样日期"
    )
    expiry_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="留样有效期"
    )
    is_expired: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否到期"
    )
    disposal_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="处置日期"
    )
    disposal_method: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="处置方式"
    )
    remark: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default="now()", nullable=False
...
class SampleRetentionLedger(BaseModel):
    ...
    sampling_item_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.sampling_order_items.id"), nullable=False
    )
    sampling_order_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.sampling_orders.id"), nullable=False
    )
    ...
    retention_location: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="存放位置"
    )
    retention_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="留样日期"
    )
    ...
    disposal_remark: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="处置备注"
    )
    reminder_sent: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否已发送提醒"
    )
    remark: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default="now()", nullable=False
...
class SamplingApprovalRecord(BaseModel):
    ...
    # 关系
    sampling_order: Mapped["SamplingOrder"] = relationship(

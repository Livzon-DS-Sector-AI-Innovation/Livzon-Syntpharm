...
class IPQCInspection(BaseModel):
    ...
    batch_record_id: Mapped[UUID | None] = mapped_column(
        nullable=True, comment="关联批次生产记录ID"
    )
    batch_record_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="批次生产记录单号"
    )
    ...
    product_specification: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="产品规格"
    )

    # 工序信息
    process_stage: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="工序/工段"
    )
    sampling_point: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="取样点"
    )
    sampling_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="取样单号"
    )
    sampling_time: Mapped[datetime | None] = mapped_column(
    ...
    sampling_unit: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="取样单位"
    )
    sampling_location: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="取样位置"
    )
    ...
    status: Mapped[str] = mapped_column(
        ...
        nullable=False,
        comment="状态：draft/submitted/workshop_approved/qc_supervisor_approved/qa_final_approved/rejected",
    )
    ...
    # 关系
    items: Mapped[list["IPQCInspectionItem"]] = relationship(
        back_populates="ipqc_inspection", lazy="selectin"
    )
    approval_records: Mapped[list["IPQCApprovalRecord"]] = relationship(
        back_populates="ipqc_inspection", lazy="selectin"
    )
...
class IPQCInspectionItem(BaseModel):
    ...
    ipqc_inspection_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.ipqc_inspections.id"), nullable=False
    )
    ...
    upper_limit: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="上限"
    )
    lower_limit: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="下限"
    )
...
class IPQCApprovalRecord(BaseModel):
    ...
    approval_level: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        comment="审批级别：1-车间工艺负责人，2-QC主管复核，3-QA终审",
    )
    ...
    # 关系
    ipqc_inspection: Mapped["IPQCInspection"] = relationship(

...
class FQCInspection(BaseModel):
    ...
    # 单据关联
    batch_record_id: Mapped[UUID | None] = mapped_column(
        nullable=True, comment="关联批生产记录ID"
    )
    batch_record_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="批生产记录编号"
    )
    batch_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="成品生产批号"
    )
    product_code: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="成品物料编码"
    )
    product_name: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="产品名称"
    )
    sampling_order_id: Mapped[UUID | None] = mapped_column(
        nullable=True, comment="入库取样单ID"
    )
    sampling_order_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="入库取样单号"
    )
    batch_quantity: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 6), nullable=True, comment="批量"
    )
    production_workshop: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="生产车间"
    )
    ...
    manufacturer: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="生产厂家"
    )
    specification: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="产品规格/包装"
    )
    ...
    status: Mapped[str] = mapped_column(
        ...
        nullable=False,
        comment="状态：draft/submitted/qc_supervisor_approved/qa_approved/final_approved/released/locked/closed",
    )
    inspection_conclusion: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="检验结论：qualified/unqualified"
    )
    ...
    batch_locked: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="批次是否锁定"
    )
    batch_lock_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="批次锁定原因"
    )
    warehouse_isolation: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否入库隔离"
    )
    ...
    release_status: Mapped[str | None] = mapped_column(
        String(32),
        nullable=True,
        comment="放行状态：pending_release/released/not_released",
    )
    release_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="放行说明"
    )

    # 复检
    reinspection_applied: Mapped[bool] = mapped_column(
        Boolean, server_default="false", nullable=False, comment="是否申请复检"
    )
    reinspection_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="复检原因"
    )
    ...
    # 检验报告
    report_no: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="检验报告书编号"
    )
    report_url: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="检验报告书URL"
    )
    ...
    # 关系
    items: Mapped[list["FQCInspectionItem"]] = relationship(
        back_populates="fqc_inspection", lazy="selectin"
    )
    approval_records: Mapped[list["FQCApprovalRecord"]] = relationship(
        back_populates="fqc_inspection", lazy="selectin"
    )
...
class FQCInspectionItem(BaseModel):
    ...
    fqc_inspection_id: Mapped[UUID] = mapped_column(
        ForeignKey("quality.fqc_inspections.id"), nullable=False
    )
    item_no: Mapped[int] = mapped_column(Integer, nullable=False, comment="项次")
    inspection_category: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="检验类别"
    )
    ...
    repeat_times: Mapped[int] = mapped_column(
        Integer, server_default="0", nullable=False, comment="复测次数"
    )
    ...
    raw_record_url: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="原始记录PDF URL"
    )
    remark: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")
...
class FQCApprovalRecord(BaseModel):
    ...
    approval_level: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="审批级别：1-QC主管，2-QA，3-质量负责人"
    )
    ...
    # 关系
    fqc_inspection: Mapped["FQCInspection"] = relationship(

...
ISSUE_TYPE_DEFINITIONS = {
    ...
    "general": ("一般问题：描述不够精确、格式不统一、部分内容遗漏但不影响整体结论的问题。"),
    "suggestion": ("建议优化：可改进的表述、可补充的说明、排版优化等不影响合规性但能提升文档质量的建议。"),
}

# ── 黄金标准提取 Prompt ──────────────────────────────────

GOLDEN_STANDARD_SYSTEM = """你是一位制药行业分析方法验证合规审核专家，\
精通 ICH Q2(R2)、中国药典分析方法验证指导原则、GMP 数据完整性要求。

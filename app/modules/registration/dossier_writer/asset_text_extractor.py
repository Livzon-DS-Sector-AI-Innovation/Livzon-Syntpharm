...
class AssetTextExtractor:
    ...
    def extract(file_path: Path) -> dict[str, Any]:
        ...
        if not file_path.exists():
            return {
                "text": "",
                "error": f"素材文件不存在，请重新上传: {file_path.name}"
            }
    ...
    def _extract_doc(file_path: Path) -> dict[str, Any]:
        ...
        # 1. 查找同目录下是否有已转换的 .docx 缓存
        cached_docx = abs_path.parent / f"{abs_path.stem}.converted.docx"
        if cached_docx.exists():
            _logger.info(f"使用缓存的 docx: {cached_docx}")
            return AssetTextExtractor._extract_docx(cached_docx)

        # 2. 查找同目录下是否有同名 .docx（可能是用户上传的 docx 版本）
        same_name_docx = abs_path.with_suffix(".docx")
        if same_name_docx.exists() and same_name_docx != abs_path:
            _logger.info(f"找到同名 docx: {same_name_docx}")
            return AssetTextExtractor._extract_docx(same_name_docx)
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if result.returncode == 0:
                    ...
                    if converted.exists():
                        ...
                        try:
                            import shutil
                            shutil.copy2(str(converted), str(cached_docx))
                            _logger.info(f"已缓存 doc 转换结果: {cached_docx}")
                        except Exception as cache_err:
                            _logger.warning(f"缓存转换结果失败: {cache_err}")
                        return AssetTextExtractor._extract_docx(converted)
                
                stderr = result.stderr.strip() if result.stderr else ""
                _logger.warning(f"libreoffice 转换失败 (rc={result.returncode}): {stderr}")
                return {
                    "text": "",
                    "error": f"LibreOffice 转换失败，请上传 docx 版本。错误: {stderr[:200] if stderr else '未知错误'}"
                }
        except subprocess.TimeoutExpired:
            _logger.warning(f"libreoffice 转换超时: {abs_path.name}")
            return {
                "text": "",
                "error": "LibreOffice 转换超时（120秒），请上传 docx 版本或稍后重试"
            }
        except FileNotFoundError:
            return {
                "text": "",
                "error": "LibreOffice 未安装，无法转换 .doc 文件。请上传 .docx 格式"
            }
        except Exception as e:
    ...
    def _extract_pdf(file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            _logger.warning(
                "PDF extraction with pdfplumber failed, falling back to OCR"
            )

...
class CoppCertificate(BaseModel):
    """COPP证书"""

    __tablename__ = "registration_copp_certificates"
    __table_args__ = (
        CheckConstraint(
            "is_expired IN ('是', '否')",
            name="ck_copp_certificates_is_expired",
        ),
        {"schema": "registration"},
    )
    ...
    applicable_countries: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="适用国家"
    )

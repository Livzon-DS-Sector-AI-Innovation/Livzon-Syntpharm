...
class DomesticApproval(BaseModel):
    """国内已获批品种"""

    __tablename__ = "registration_domestic_approvals"
    __table_args__ = (
        CheckConstraint(
            "is_expired IN ('是', '否')",
            name="ck_domestic_approvals_is_expired",
        ),
        {"schema": "registration"},
    )
    ...
    registration_no: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="登记号"
    )
    ...
    production_workshop: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="生产车间"
    )
    product_validity: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="产品有效期"
    )
    storage_condition: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="贮存条件"
    )

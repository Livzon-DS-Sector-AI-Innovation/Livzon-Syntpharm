...
def make_input(description: str = "测试隐患", **kwargs) -> Any:
...
class TestSchemas:
    ...
    def test_output_from_valid_dict(self) -> Any:
        """有效字典应成功构建输出。"""
    ...
    def test_output_invalid_enum_rejected(self) -> Any:
        """无效枚举值应被 Pydantic 拒绝。"""
        with pytest.raises(ValueError):
            HazardIdentificationOutput(**{**VALID_OUTPUT_DICT, "hazard_type": "invalid_type"})

    def test_plugin_config_defaults(self) -> Any:
        """默认配置应合理。"""
        config = PluginConfig()
...
class TestPrompts:
    ...
    def test_work_rules_contains_all_sections(self) -> Any:
        sections = [
            "隐患分类判定规则",
            "隐患类别判定规则",
            "隐患级别判定规则",
    ...
    def test_work_rules_contains_shutdown_criterion(self) -> Any:
        """隐患级别判定规则应包含停产整改判断（新增最高优先级）。"""
        assert "停产整改判断" in WORK_RULES
        assert "全厂停产或全车间停产" in WORK_RULES
        assert "局部停产" in WORK_RULES

    def test_output_format_is_valid_json_template(self) -> Any:
    ...
    def test_fewshot_examples_have_valid_enums(self) -> Any:
        """Few-shot 示例的枚举值应合法。"""
    ...
    def test_fewshot_examples_have_two_tier_rectification(self) -> Any:
    ...
    def test_build_context_text(self) -> Any:
    ...
    def test_build_full_prompt_includes_all_sections(self) -> Any:
...
class TestRuleEngine:
    ...
    def test_invalid_enum_detected(self) -> Any:
        ...
        output = HazardIdentificationOutput.model_construct(**{**VALID_OUTPUT_DICT, "hazard_type": "invalid"})
        result = self.engine.validate(make_input(), output)
        assert not result.is_valid
        assert any("隐患分类" in e for e in result.errors)

    def test_short_key_defect_detected(self) -> Any:
        output = make_output(key_defect="很短")
        result = self.engine.validate(make_input(), output)
        assert not result.is_valid
        assert any("过短" in e for e in result.errors)

    def test_empty_rectification_detected(self) -> Any:
        output = make_output(rectification_suggestion=RectificationSuggestion(corrective="", preventive=""))
        result = self.engine.validate(make_input(), output)
        assert not result.is_valid
        assert len([e for e in result.errors if "不能为空" in e]) == 2

    def test_basis_too_short_detected(self) -> Any:
    ...
    def test_banned_phrase_detected(self) -> Any:
        output = make_output(
...
class TestAutoCorrect:
    ...
    def test_strips_key_defect(self) -> Any:
        output = make_output(key_defect="  多余的空白描述  ")
        corrected = auto_correct(output)
        assert corrected.key_defect == "多余的空白描述"

    def test_fills_empty_rectification(self) -> Any:
        output = make_output(rectification_suggestion=RectificationSuggestion(corrective="具体措施", preventive=""))
        corrected = auto_correct(output)
...
class MockAIService:
    ...
    async def chat_parsed(self, messages, expected_keys, temperature=0.1) -> Any:
        self.call_count += 1
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_identify_basic(self) -> Any:
    ...
    @pytest.mark.asyncio
    async def test_identify_uses_vision_when_photos_available(self) -> Any:
        from app.modules.safety.ai_hazard_identification.plugin import (
            AIHazardIdentifier,
        )
    ...
    @pytest.mark.asyncio
    async def test_identify_ai_failure_raises(self) -> Any:
        from app.modules.safety.ai_hazard_identification.plugin import (
            AIHazardIdentifier,
            IdentificationError,
        )
    ...
    @pytest.mark.asyncio
    async def test_identify_batch(self) -> Any:
        from app.modules.safety.ai_hazard_identification.plugin import (
            AIHazardIdentifier,
        )
    ...
    @pytest.mark.asyncio
    async def test_identify_validates_output_on_failure(self) -> Any:
        from app.modules.safety.ai_hazard_identification.plugin import (
            AIHazardIdentifier,
            IdentificationError,
        )
...
class TestQualityBenchmarks:
    ...
    def test_example1_electric_box(self) -> Any:
    ...
    def test_example2_height_work(self) -> Any:
        """示例2: 高处作业未佩戴安全带。"""
        output = HazardIdentificationOutput(
    ...
    def test_example3_blocked_fire_exit(self) -> Any:
        """示例3: 消防通道堵塞。"""
        output = HazardIdentificationOutput(
    ...
    def test_example4_permit_issue(self) -> Any:
        """示例4: 动火票证审批不完整。"""
        output = HazardIdentificationOutput(

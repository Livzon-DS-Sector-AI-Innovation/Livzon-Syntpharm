...
class AIHazardIdentifier:
    ...
    # ── 内部方法 ──
    async def _call_text_ai(self, input_data: HazardIdentificationInput) -> dict[str, Any]:
        """纯文本模式 AI 调用。"""
        context = build_context_text(
            ...
            discovered_at=(input_data.discovered_at.strftime("%Y-%m-%d %H:%M") if input_data.discovered_at else None),
        )
    ...
    async def _call_vision_ai(self, input_data: HazardIdentificationInput) -> dict[str, Any]:
        """多模态 AI 调用（带图片）。"""

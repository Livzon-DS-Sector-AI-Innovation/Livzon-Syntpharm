...
class ProductDossier(BaseModel):
    ...
    # 文件路径
    source_templates_path: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="原始模板目录路径"
    )
    working_path: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="工作副本目录路径"
    )
    assets_path: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="素材目录路径"
    )
    outputs_path: Mapped[str | None] = mapped_column(
...
class DossierChapter(BaseModel):
    ...
    working_file: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="工作副本文件名"
    )
    paragraph_start: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="起始段落索引"
    )
    paragraph_end: Mapped[int | None] = mapped_column(

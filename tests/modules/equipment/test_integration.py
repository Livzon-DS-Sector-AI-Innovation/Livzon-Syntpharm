...
async def test_equipment_lifecycle(auth_client: AsyncClient):
    ...
    equipment_response = await auth_client.post(
    ...
    assert equipment_data["equipment_no"] == equipment_no
    ...
    delete_response = await auth_client.delete(equip_url)
    assert delete_response.status_code == 200

    # 7. 验证设备已被删除（软删除后返回404）
    detail_response = await auth_client.get(equip_url)
    assert detail_response.status_code == 404


async def test_equipment_number_generation(auth_client: AsyncClient):
    """测试设备编号自动生成（由调用方生成后传入）"""
    uid = _uid()
    cat_code = f"LXJ-{uid}"
    loc_code = f"WS-{uid}"
    ...
    for i in range(1, 4):
        ...
        assert response.json()["data"]["equipment_no"] == eq_no


async def test_equipment_filter(auth_client: AsyncClient):
    """测试设备筛选功能"""
    uid = _uid()
    cat_code = f"RF-{uid}"
    loc_code = f"WS-{uid}"
...
async def test_create_equipment_with_nonexistent_category(auth_client: AsyncClient):
...
async def test_create_duplicate_category_code(auth_client: AsyncClient):
    """测试创建重复的分类代码（应返回409）"""
    uid = _uid()
    cat_code = f"DUP-{uid}"

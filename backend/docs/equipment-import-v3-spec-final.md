...
## 3. 智能同步功能 (Sync Mode) - 新增需求

### 3.1 业务背景
设备模块的成本、净值等财务数据每月都会在 Excel 中更新。用户需要一个“同步”按钮，在不丢失历史工单、校准记录的前提下，将 Excel 的最新数据覆盖到数据库。

### 3.2 核心逻辑：Upsert (更新或插入)
与 v3 的“混合导入”不同，同步模式不以创建新数据为主，而是以**维护数据一致性**为主。

#### 唯一性约束
- **主键**: `(asset_no, department_id, location_id)`
- **匹配策略**: 
  1. 优先匹配完整组合。
  2. 若组合不存在但 `asset_no` 存在，视为**资产迁移**（更新其部门和位置）。
  3. 若 `asset_no` 完全不存在，视为**新增设备**。
  4. 若数据库有但 Excel 无，视为**停用/软删除**。

### 3.3 字段同步范围
| 字段类型 | 同步行为 | 说明 |
| :--- | :--- | :--- |
| **基础信息** | 强制覆盖 | 名称、型号、制造商、分类等 |
| **财务数据** | 强制覆盖 | **当前成本、账面净值** (核心需求) |
| **位置归属** | 强制覆盖 | 部门、位置文本 |
| **状态信息** | 强制覆盖 | 设备状态、报废状态 |
| **关联数据** | **禁止修改** | 工单、校准、巡检、备件领用记录保持不变 |

### 3.4 API 接口设计
- **Endpoint**: `POST /api/v1/equipment/sync-excel`
...
### 3.5 实施决策
- **Service 层**: 新增 `sync_equipments_from_excel` 函数。
- **事务处理**: 整个同步过程在一个数据库事务中完成，确保原子性。
- **性能优化**: 使用批量 `UPDATE` 语句而非逐条操作，提高同步速度。

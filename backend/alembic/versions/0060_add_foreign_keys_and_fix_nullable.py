...
def upgrade() -> None:
    ...
    op.alter_column('graph_knowledge_edges', 'relation_type',
                    ...
                    comment='图片号 ：cites/supplements/replaces/belongs_to/related_to/conflicts_with',
                    schema='safety')
    op.alter_column('graph_knowledge_edges', 'status',
                    existing_type=sa.String(length=32),
                    existing_nullable=False,
                    existing_server_default=sa.text("'ai_generated'::character varying"),
                    comment='牾牿者：ae_generated/human_confirmed/human_deleted/human_added',
                    schema='safety')
    ...
    op.alter_column('graph_knowledge_nodes', 'entity_type',
                    ...
                    existing_server_default=False,
                    comment='图片号 ：equipment/condition/location/operation/material/standard',
                    schema='safety')
    op.alter_column('graph_knowledge_nodes', 'status',
                    existing_type=sa.String(length=32),
                    existing_nullable=False,
                    existing_server_default=sa.text("'ai_generated'::character varying"),
                    comment='牾牾耇ﺟai_generated/human_confirmed/deprecated/merged',
                    schema='safety')
...
def downgrade() -> None:
    ...
    op.alter_column('graph_knowledge_nodes', 'status', existing_type=sa.String(length=32), existing_nullable=False, existing_server_default=sa.text("'ai_generated'::character varying"), comment=�g��牿者：ae_generated/human_confirmed/human_deleted/human_added', schema='safety')
    op.alter_column('graph_knowledge_nodes', 'entity_type', existing_type=sa.String(length=32), existing_nullable=True, comment='图片号', schema='safety')
    ...
    op.alter_column('graph_knowledge_edges', 'status', existing_type=sa.String(length=32), existing_nullable=False, existing_server_default=sa.text("'ai_generated'::character varying"), comment='牾牿者', schema='safety')
    op.alter_column('graph_knowledge_edges', 'relation_type', existing_type=sa.String(length=32), existing_nullable=False, comment=�g��牿者牾', schema='safety')

...
def create_mock_excel(rows):
    df = pd.DataFrame(rows, columns=[
        '资产编号', '设备名称', '实物所在部门', '实物所在地点', 
        '当前成本', '帐面净值', '型号', '制造商', '启用日期'
    ])
    output = BytesIO()
...
async def test_sync_updates_cost_and_value(db: AsyncSession, seed_departments_and_locations):
    ...
    equip = Equipment(
        asset_no="TEST001", name="旧设备", department_id=d.id, 
        current_cost=100.0, book_value=50.0, is_deleted=False
    )
    db.add(equip)
    await db.commit()
    ...
    excel_data = create_mock_excel([{
        '资产编号': 'TEST001', '设备名称': '新设备', '实物所在部门': '201车间',
...
async def test_sync_soft_deletes_missing_assets(db: AsyncSession, seed_departments_and_locations):
    ...
    assert deleted_equip.is_deleted == True
...
async def test_sync_threshold_fuse(db: AsyncSession, seed_departments_and_locations):
    ...
    excel_data = create_mock_excel([{'资产编号': 'FUSE0', '设备名称': '唯一', '实物所在部门': '201车间', '实物所在地点': '-', '当前成本': 0, '帐面净值': 0, '型号': '', '制造商': '', '启用日期': None}])

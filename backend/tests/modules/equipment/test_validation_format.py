...
def test_validation_py_ruff_format():
    """验证 validation.py 通过 ruff format 检查。
    
    这个测试确保文件格式符合项目规范，防止 CI 中的 ruff format check 失败。
    """
    validation_file = Path(__file__).parent.parent.parent.parent / "app" / "modules" / "equipment" / "service" / "validation.py"
    ...
    assert result.returncode == 0, (
        f"validation.py 格式不符合规范:\n"
        f"stdout: {result.stdout}\n"
        f"stderr: {result.stderr}"
    )


def test_validation_py_no_trailing_whitespace():
...
def test_validation_py_ends_with_single_newline():

...
async def test_tb04_audit_log_created(db_session, seed_basic_data):
    ...
    logs = await db_session.execute(select(EquipmentSyncLog).where(EquipmentSyncLog.file_name == "audit_test.xlsx"))

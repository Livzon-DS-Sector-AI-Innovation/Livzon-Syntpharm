...
async def test_tb01_exact_match_updates_cost(db_session, seed_basic_data):
    ...
    d = dept.scalar_one()
    equip = Equipment(asset_no="T001", name="OldName", department_id=d.id, current_cost=100.0, book_value=50.0, is_deleted=False)
    db_session.add(equip)
    await db_session.commit()

    # Act: 上传包含新成本的 Excel
    excel_data = _create_excel([{
        '资产编号': 'T001', '设备名称': 'NewName', '实物所在部门': '201车间', 
        '实物所在地点': '-', '当前成本': 200.0, '帐面净值': 100.0, 
        '型号': '', '制造商': '', '启用日期': None
    }])
...
async def test_tb04_threshold_fuse(db_session, seed_basic_data):
    ...
    excel_data = _create_excel([{'资产编号': 'F0', '设备名称': 'One', '实物所在部门': '201车间', '实物所在地点': '-', '当前成本': 0, '帐面净值': 0, '型号': '', '制造商': '', '启用日期': None}])
...
async def test_tb04_audit_log_created(db_session, seed_basic_data):
    ...
    dept = await db_session.execute(select(HrDepartment).where(HrDepartment.name == "201车间"))
    d = dept.scalar_one()
    excel_data = _create_excel([{'资产编号': 'LOG01', '设备名称': 'LogTest', '实物所在部门': '201车间', '实物所在地点': '-', '当前成本': 10, '帐面净值': 5, '型号': '', '制造商': '', '启用日期': None}])

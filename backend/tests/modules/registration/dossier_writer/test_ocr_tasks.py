...
async def test_ocr_task_repository_list_by_asset(db_session, sample_asset):
    ...
    task1 = await repo.create_task(sample_asset.id, sample_asset.chapter_id, "preview_extraction")
    task2 = await repo.create_task(sample_asset.id, sample_asset.chapter_id, "split_preview")

...
async def test_create_and_get_rd_project(db_session):
    proj = await service.create_rd_project(db_session, RdProjectCreate(
...
async def test_create_deliverable(db_session):
    ...
    deliverable = await service.create_rd_stage_deliverable(db_session, RdStageDeliverableCreate(
        project_id=proj.id,
        stage="development",
...
async def test_get_deliverable(db_session):
    ...
    d = await service.create_rd_stage_deliverable(db_session, RdStageDeliverableCreate(
        project_id=proj.id,
        stage="development",
        deliverable_type="report",
        title="工艺开发报告",
...
async def test_create_pilot_study(db_session):
    ...
    study = await service.create_pilot_study(db_session, RdPilotStudyCreate(
        project_id=proj.id,
        batch_no="PILOT-001",
    ))
...
async def test_create_experiment_log(db_session):
    ...
    log = await service.create_experiment_log(db_session, RdExperimentLogCreate(
        project_id=proj.id,
        title="结晶条件优化实验",

...
class TestUnitConsumptionCalculation:
    ...
    def test_normal_calculation(self):
        """正常计算"""
        result = calculate_unit_consumption(30000.0, 12000)
        assert result == 2.5

    def test_precision(self):
        """精度测试"""
        result = calculate_unit_consumption(10000.0, 3333)
        assert result == 3.0003  # 10000/3333 = 3.00030003...

    def test_zero_production_raises_error(self):
        """产量为0应抛出异常"""
        with pytest.raises(ValueError, match="产量必须为正整数"):
            calculate_unit_consumption(1000.0, 0)

    def test_negative_production_raises_error(self):
        """负数产量应抛出异常"""
        with pytest.raises(ValueError, match="产量必须为正整数"):
            calculate_unit_consumption(1000.0, -100)

    def test_single_unit_production(self):
        """产量为1的边界情况"""
        result = calculate_unit_consumption(500.0, 1)
        assert result == 500.0
...
class TestAIPromptConstruction:
    ...
    def test_prompt_without_target(self):
        """无目标值时的 Prompt"""
        analysis_data = {
            "workshop_name": "装配车间A",
            "analysis_month": "2026-08",
    ...
    def test_prompt_json_format_requirement(self):

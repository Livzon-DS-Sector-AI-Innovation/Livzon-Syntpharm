...
for acq_i in ["EHVI", "MOUCB", "MOGreedy"]:
    for seed_i in np.arange(0, 5):
        ...
        columns_regression = columns_regression.drop(
            [sort_column, "objective_conversion", "objective_selectivity"]
        ).tolist()
        ...
        if not os.path.exists(f"./results_mordred/{label_benchmark}"):
            ...
            shutil.move(
                f"pred_{label_benchmark}", f"results_mordred/pred_{label_benchmark}"
            )
            shutil.move(
                f"results_{label_benchmark}",
                f"results_mordred/results_{label_benchmark}",
            )

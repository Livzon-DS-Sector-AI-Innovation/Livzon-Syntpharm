...
for campaign in [
    ...
    sns.scatterplot(
        x=np.arange(1, 8),
        y=av_uncertainties_yield,
        ax=ax[0],
        label="average_uncertainty_yield",
    )
    sns.scatterplot(
        x=np.arange(1, 8),
        y=av_uncertainties_ee,
        ax=ax[0],
        label="average_uncertainty_ee",
    )
    plt.title(f"{campaign}", loc="center")
    sns.scatterplot(
        x=np.arange(1, 8),
        y=max_uncertainties_yield,
        ax=ax[1],
        label="max_uncertainty_yield",
    )
    sns.scatterplot(
        x=np.arange(1, 8), y=max_uncertainties_ee, ax=ax[1], label="max_uncertainty_ee"
    )
    ...
    ax[0].legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.2), fancybox=True, shadow=True
    )
    ax[1].legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.2), fancybox=True, shadow=True
    )
    ...
    plt.title(f"{campaign}", loc="center")
    sns.scatterplot(
        x=np.arange(1, 8), y=av_eis_yield, ax=ax[2], label="average_EI_yield"
    )

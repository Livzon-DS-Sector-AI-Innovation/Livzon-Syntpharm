...
for feat in ["ohe", "dft", "mordred", "random"]:
    for acq in ["EHVI", "MOUCB", "MOGreedy"]:
        df_i = pd.read_csv(
            f"../results_{feat}/results_benchmark_{feat}_acq_{acq}_batch_{batch}_seed_0.csv"
        )
        ...
        for seed_i in range(0, 5):
            df_j = pd.read_csv(
                f"../results_{feat}/results_benchmark_{feat}_acq_{acq}_batch_{batch}_seed_{seed_i}.csv"
            )

...
feat = "dft"
fig, ax = plt.subplots(figsize=(15.0, 4.0), dpi=500, nrows=1, ncols=4)
...
for batch in [1, 2, 3, 5]:
    ...
    for acq in ["EHVI", "MOUCB", "MOGreedy"]:
        ...
        ax[batch_count].plot(
            n_exp, hypervol_avg, color=colors[color_i], lw=2.5, label=acq.upper()
        )
        ax[batch_count].fill_between(
            x=n_exp,
            y1=hypervol_avg,
            y2=hypervol_max,
        ...
        ax[batch_count].scatter(
            n_exp, hypervol_avg, marker="o", s=0.0, color=colors[color_i]
        )

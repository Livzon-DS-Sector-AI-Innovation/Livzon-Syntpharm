...
async def get_inspection_template_by_name(db: AsyncSession, name: str) -> InspectionTemplate | None:
    """根据名称获取巡检模板。"""
    from app.modules.equipment.models.inspection_template import InspectionTemplate

    result = await db.execute(select(InspectionTemplate).where(InspectionTemplate.name == name))
    return result.scalar_one_or_none()

...
class SparePart(BaseModel):
    ...
    category: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="备件分类")
    default_supplier: Mapped[str | None] = mapped_column(String(200), nullable=True, comment="默认供应商")
    unit_price: Mapped[float | None] = mapped_column(Numeric(12, 2), nullable=True, comment="参考单价")
...
class EquipmentSparePart(BaseModel):
    ...
    quantity: Mapped[int] = mapped_column(Integer, default=1, comment="该设备需要的数量")
...
class SparePartStock(BaseModel):
    ...
    warehouse_location: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="库位")
    current_qty: Mapped[int] = mapped_column(Integer, default=0, server_default="0", comment="当前库存")
    safety_qty: Mapped[int] = mapped_column(Integer, default=0, server_default="0", comment="安全库存")
    min_order_qty: Mapped[int] = mapped_column(Integer, default=1, server_default="1", comment="最小采购批量")
...
class SparePartTransaction(BaseModel):
    ...
    transaction_type: Mapped[str] = mapped_column(String(20), comment="类型：入库/出库/盘点调整")

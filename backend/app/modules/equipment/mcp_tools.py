...
async def list_work_orders(
    ...
    if status:
        ...
        if status not in valid_statuses:
            raise ValueError(f"无效的工单状态 '{status}'，可选值：{' / '.join(valid_statuses)}")
...
async def submit_inspection(
    ...
    for item in check_items:
        ...
        if result not in valid_results:
            raise ValueError(f"检查项 '{item_name}' 的 result '{result}' 无效，可选值：正常 / 异常 / 跳过")
        # 异常项必须填写实际值或备注
        if result == "异常" and not item.get("actual_value") and not item.get("remark"):
            raise ValueError(f"检查项 '{item_name}' 结果为异常，必须填写 actual_value 或 remark")
    ...
    return {
        ...
        "all_done": all_done,
        "message": (f"已提交 {len(submitted)} 项检查结果，" + ("巡检任务已完成" if all_done else "还有待检设备")),
    }

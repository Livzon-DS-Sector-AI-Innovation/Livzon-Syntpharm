...
from app.modules.safety.models import (  # type: ignore[attr-defined]
    ScheduledTask,
    ScheduledTaskLog,
)
...
class ScheduledTaskService:
    ...
    async def get_tasks(
        ...
        return await self.repo.get_scheduled_tasks(skip, limit, is_enabled, search)  # type: ignore[attr-defined,no-any-return]

    async def get_task(self, task_id: uuid.UUID) -> ScheduledTask | None:
        return await self.repo.get_scheduled_task_by_id(task_id)  # type: ignore[attr-defined]
    ...
    async def run_task_now(self, task_id: uuid.UUID) -> ScheduledTaskLog | None:
        ...
        logs, _ = await self.repo.get_task_logs(task_id, skip=0, limit=1)  # type: ignore[attr-defined]
        return logs[0] if logs else None

    async def get_logs(self, task_id: uuid.UUID, skip: int = 0, limit: int = 20) -> tuple[list[ScheduledTaskLog], int]:
        return await self.repo.get_task_logs(task_id, skip, limit)  # type: ignore[attr-defined,no-any-return]
    ...
    async def preview_card(data: CardPreviewRequest) -> dict[str, Any]:
        ...
        if not template:
            template = build_default_template([ds.model_dump() for ds in data.data_sources])

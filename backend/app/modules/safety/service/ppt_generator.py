...
THEMES: dict[str, dict[str, Any]] = {
    "professional": {
        ...
        "title_color": RGBColor(0x1A, 0x56, 0xDB),     # 蓝色标题
        "text_color": RGBColor(0x33, 0x33, 0x33),      # 深灰正文
        "accent_color": RGBColor(0x1A, 0x56, 0xDB),    # 蓝色强调
        "subtitle_color": RGBColor(0x66, 0x66, 0x66),  # 中灰副标题
    ...
    "modern": {
        "bg_color": RGBColor(0x1E, 0x1E, 0x2E),        # 深色背景
        "title_color": RGBColor(0xFF, 0xFF, 0xFF),      # 白色标题
        "text_color": RGBColor(0xCC, 0xCC, 0xCC),       # 浅灰正文
        "accent_color": RGBColor(0x00, 0xD4, 0xAA),     # 青绿强调
        "subtitle_color": RGBColor(0x99, 0x99, 0x99),   # 灰色副标题
        "title_font": "Microsoft YaHei",
        "body_font": "Microsoft YaHei",
        "title_size": Pt(28),
        "body_size": Pt(16),
    ...
    "minimal": {
        "bg_color": RGBColor(0xFF, 0xFF, 0xFF),        # 白色背景
        "title_color": RGBColor(0x00, 0x00, 0x00),      # 黑色标题
        "text_color": RGBColor(0x44, 0x44, 0x44),       # 灰色正文
        "accent_color": RGBColor(0x66, 0x66, 0x66),     # 灰色强调
        "subtitle_color": RGBColor(0x88, 0x88, 0x88),   # 浅灰副标题
        "title_font": "Microsoft YaHei",
        "body_font": "Microsoft YaHei",
        "title_size": Pt(28),
        "body_size": Pt(16),
...
{article.content[:3000] if article.content else ""}"""
    ...
    def _fallback_outline(self, article: SafetyKnowledgeArticle) -> dict[str, Any]:
        ...
        slides.append({
            "title": "总结",
            "content": "本文档共 {} 章，涵盖了相关法规的核心内容。".format(len(chapter_matches) if chapter_matches else "多"),
            "type": "summary",
        })
                ...
                if len(content_text) > MAX_CONTENT_CHARS and slide_type != "summary":
                    ...
                    for line in lines:
                        ...
                        if current_length + line_length > MAX_CONTENT_CHARS and current_page_lines:
                            ...
                            page_title = f"{title_text} ({page_num}/{((len(content_text) + MAX_CONTENT_CHARS - 1) // MAX_CONTENT_CHARS)})" if page_num > 1 else title_text
                    ...
                    if current_page_lines:
                        ...
                        self._add_content_slide(prs, page_title, page_content, theme, is_summary=(slide_type == "summary"))
                else:
    ...
    def _add_content_slide(
        ...
        if content_text:
            ...
            txBox2 = slide.shapes.add_textbox(left, content_top, width, content_height)
            tf2 = txBox2.text_frame
            tf2.word_wrap = True

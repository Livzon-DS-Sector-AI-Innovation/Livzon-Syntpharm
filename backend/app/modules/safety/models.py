...
class SafetyCheck(BaseModel):
    ...
    check_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="检查日期")
    ...
    # 关系
    hazards: Mapped[list[HazardReport]] = relationship("HazardReport", back_populates="safety_check", lazy="selectin")
...
class HazardReport(BaseModel):
    ...
    major_hazard_basis: Mapped[str | None] = mapped_column(Text, nullable=True, comment="隐患判定依据（AI）")
    key_defect: Mapped[str | None] = mapped_column(Text, nullable=True, comment="隐患描述（AI）")
    defect_substance: Mapped[str | None] = mapped_column(
        String(32),
        nullable=True,
        comment="缺陷实质评估: substantive / procedural / uncertain",
    )
    defect_substance_reasoning: Mapped[str | None] = mapped_column(Text, nullable=True, comment="缺陷实质评估理由")
    defect_photos: Mapped[str | None] = mapped_column(Text, nullable=True, comment="缺陷图片JSON数组")
    ...
    corrective_preventive_measures: Mapped[str | None] = mapped_column(Text, nullable=True, comment="AI整改建议")
    rectification_reply: Mapped[str | None] = mapped_column(Text, nullable=True, comment="整改回复内容")
    deadline: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="整改期限")
    actual_completion_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="整改完成时间"
    )
    rectification_photos: Mapped[str | None] = mapped_column(Text, nullable=True, comment="整改后图片JSON数组")
    ...
    ai_error_message: Mapped[str | None] = mapped_column(Text, nullable=True, comment="AI 脚本执行错误信息")
    ...
    rectification_notify_error: Mapped[str | None] = mapped_column(Text, nullable=True, comment="整改通知失败原因")
    review_notified_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="复核通知最近发送时间"
    )
    review_notified_level: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="复核通知级别: 1/2/3")
    review_notify_status: Mapped[str | None] = mapped_column(
        String(16), nullable=True, comment="复核通知状态: success / failed"
    )
    review_notify_error: Mapped[str | None] = mapped_column(Text, nullable=True, comment="复核通知失败原因")

    # 关系
    safety_check: Mapped[SafetyCheck | None] = relationship("SafetyCheck", back_populates="hazards")
...
class Accident(BaseModel):
    ...
    accident_level: Mapped[str] = mapped_column(String(32), nullable=False, default="general", comment="事故等级")
    happened_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="发生时间")
    ...
    corrective_action_responsible: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="CAPA责任人")
    ...
    verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="验证时间")
    status: Mapped[str] = mapped_column(
        String(32),
        default="reported",
    ...
    reported_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="报告时间")
...
class SafetyTraining(BaseModel):
    ...
    training_type: Mapped[str] = mapped_column(String(32), nullable=False, default="annual", comment="培训类型")
    training_mode: Mapped[str] = mapped_column(String(16), nullable=False, default="offline", comment="培训方式")
    ...
    training_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="培训日期")
    ...
    course_material_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="课程资料路径")
    status: Mapped[str] = mapped_column(
        String(32),
        default="draft",
        server_default="draft",
    ...
    # 关系
    records: Mapped[list[TrainingRecord]] = relationship("TrainingRecord", back_populates="training", lazy="selectin")
...
class TrainingRecord(BaseModel):
    ...
    certificate_file_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="证书文件路径")
    notes: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")
...
class HazardIdentification(BaseModel):
    ...
    attachment_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="岗位资料附件路径")
    ...
    regulation_name: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="引用的安全操作规程名称")
    ...
    operation_frequency: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="作业频次")
    ...
    inherent_risk_level: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="固有风险等级")
    inherent_risk_label: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="固有风险等级中文名")

    # ── 脚本4 输出：现有控制措施（AI → 人工审核） ──
    existing_engineering_controls: Mapped[str | None] = mapped_column(Text, nullable=True, comment="现有工程控制措施")
    existing_management_controls: Mapped[str | None] = mapped_column(Text, nullable=True, comment="现有管理控制措施")
    existing_ppe: Mapped[str | None] = mapped_column(Text, nullable=True, comment="现有个人防护措施")
    existing_emergency_measures: Mapped[str | None] = mapped_column(Text, nullable=True, comment="现有应急措施")
    ...
    residual_risk_level: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="残余风险等级")
    residual_risk_label: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="残余风险等级中文名")

    # ── 脚本6 输出：建议措施（AI → 人工审核） ──
    needs_recommendation: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="是否需提出建议措施（是/否/待人工确认）"
    )
    recommendation_type: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="建议措施类型")
    recommendation_content: Mapped[str | None] = mapped_column(Text, nullable=True, comment="建议措施内容")
    ...
    post_risk_level: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="建议措施后风险等级")
    post_risk_label: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="建议措施后风险等级中文名")
...
class OperationRegulation(BaseModel):
    ...
    document_original_name: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="文档原始文件名")
    position: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="岗位（达托/达巴，逗号分隔）")
    notes: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")

    # ── 标准化生成字段 ──
    content: Mapped[str | None] = mapped_column(Text, nullable=True, comment="标准化 Markdown 内容（9 章完整操规）")
    status: Mapped[str] = mapped_column(
        String(20),
...
class RegulationRevision(BaseModel):
    ...
    old_document_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="旧文档路径")
    ...
    revision_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="修订时间")
    ...
    new_document_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="新文档路径")
    notes: Mapped[str | None] = mapped_column(Text, nullable=True, comment="备注")

    # 关系
    regulation: Mapped[OperationRegulation] = relationship("OperationRegulation", back_populates="revisions")
...
class SpecialOperationPersonnel(BaseModel):
    ...
    certificate_type: Mapped[str] = mapped_column(String(32), nullable=False, comment="证书类型（对应8种特殊作业）")
    certificate_number: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="证书编号")
    issuing_authority: Mapped[str | None] = mapped_column(String(200), nullable=True, comment="发证机关")
    issue_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="发证日期")
...
class SpecialOperationPermit(BaseModel):
    ...
    operation_type: Mapped[str] = mapped_column(String(32), nullable=False, comment="作业类型（8种）")
    ...
    applicant_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="申请人姓名")
    work_leader_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="作业负责人姓名")
    operator_names: Mapped[str | None] = mapped_column(Text, nullable=True, comment="作业人员姓名（逗号分隔）")
    guardian_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="监护人姓名")
    ...
    emergency_equipment: Mapped[str | None] = mapped_column(Text, nullable=True, comment="应急消防器材")
...
class SpecialOperationReport(BaseModel):
    ...
    applicant_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="报备申请人姓名")
    approver_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="审批人姓名")
    ...
    is_critical_reason: Mapped[str | None] = mapped_column(Text, nullable=True, comment="关键作业判定理由")
...
class DailyRiskReport(BaseModel):
    ...
    operation_description: Mapped[str] = mapped_column(Text, nullable=False, comment="风险作业描述")
    ...
    responsible_person: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="作业负责人")
...
class EhsChange(BaseModel):
    ...
    # ── 核心标识 ──
    change_no: Mapped[str] = mapped_column(String(64), nullable=False, comment="变更编号")
    ...
    department: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="申请部门")
    location_unit: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="所在单元/装置")
    description: Mapped[str | None] = mapped_column(Text, nullable=True, comment="变更描述（变更前/变更后对比）")
    technical_basis: Mapped[str | None] = mapped_column(Text, nullable=True, comment="变更技术依据")
    ...
    expected_effect: Mapped[str | None] = mapped_column(Text, nullable=True, comment="预期效果")
    ...
    # ── JSON 辅助字段 ──
    equipment_tags: Mapped[list[Any] | None] = mapped_column(JSON, nullable=True, comment="关联设备位号 JSON数组")
    ...
    # ── 关系 ──
    applicant: Mapped[User | None] = relationship("app.platform.identity.models.User", foreign_keys=[applicant_id])
    linked_safety_check: Mapped[SafetyCheck | None] = relationship("SafetyCheck", foreign_keys=[linked_safety_check_id])
...
class OhHazardMonitor(BaseModel):
    ...
    # ── 核心标识 ──
    monitor_no: Mapped[str] = mapped_column(String(64), nullable=False, comment="监测编号")

    # ── 监测点信息 ──
    workplace: Mapped[str] = mapped_column(String(255), nullable=False, comment="监测场所/车间")
    location: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="具体监测点位")
    equipment_info: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="关联设备/岗位")
    ...
    detection_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="检测日期")
    detection_agency: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="检测机构")
    ...
    # ── 责任人 ──
    inspector_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="检测人员")
    verifier_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="验证人员")
...
class OhHealthExam(BaseModel):
    ...
    department: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="部门")
    job_position: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="岗位")
    ...
    # ── 体检信息 ──
    exam_agency: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="体检机构")
    scheduled_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="计划体检日期"
    )
    exam_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="实际体检日期")
    report_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="报告日期")

    # ── 暴露因素关联 ──
    hazard_factors: Mapped[list[Any] | None] = mapped_column(JSON, nullable=True, comment="关联的危害因素列表 [string]")

    # ── 个人体检结论 ──
    overall_conclusion: Mapped[str | None] = mapped_column(String(32), nullable=True, comment="综合体检结论")
    ...
    # ── JSON：异常处置记录 ──
    abnormality_records: Mapped[list[Any] | None] = mapped_column(JSON, nullable=True, comment="异常处置记录 JSON数组")

    # ── JSON：附件 ──
    attachments: Mapped[list[Any] | None] = mapped_column(JSON, nullable=True, comment="附件列表")
...
class Contractor(BaseModel):
    ...
    legal_representative: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="法定代表人")
    ...
    qualification_cert_no: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="资质证书编号")
    qualification_expiry: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="资质有效期至"
    )
    safety_license_no: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="安全生产许可证编号")
    ...
    safety_officer_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="安全负责人")
    safety_officer_phone: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="安全负责人电话")
...
class ContractorWorkRecord(BaseModel):
    ...
    work_location: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="施工地点")
    planned_start: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="计划开始时间")
    planned_end: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="计划结束时间")
    actual_start: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="实际开始时间"
    )
    actual_end: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="实际结束时间")
    permit_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("safety.special_operation_permits.id"),
        nullable=True,
    ...
    leading_person: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="带班负责人")

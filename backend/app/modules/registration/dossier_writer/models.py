...
class ProductDossier(BaseModel):
    ...
    sterile_type: Mapped[str] = mapped_column(String(50), nullable=False, comment="无菌/非无菌")
    manufacturer: Mapped[str] = mapped_column(String(300), nullable=False, comment="生产商")
    template_original_product_name: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="模板原品种名称"
    )
    template_original_manufacturer: Mapped[str | None] = mapped_column(
        String(300), nullable=True, comment="模板原生产商"
    )

    # 文件路径
    source_templates_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="原始模板目录路径")
    working_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="工作副本目录路径")
    assets_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="素材目录路径")
    outputs_path: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="导出目录路径")
    ...
    parse_error: Mapped[str | None] = mapped_column(Text, nullable=True, comment="解析错误信息")
...
class DossierTemplate(BaseModel):
    ...
    original_filename: Mapped[str] = mapped_column(String(300), nullable=False, comment="原始文件名")
...
class DossierChapter(BaseModel):
    ...
    # 章节信息
    chapter_code: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="章节编号，如 3.2.S.1.1")
    chapter_title: Mapped[str] = mapped_column(String(500), nullable=False, comment="章节标题")
    level: Mapped[int] = mapped_column(Integer, nullable=False, default=1, server_default="1", comment="层级深度")
    sort_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0", comment="排序序号")

    # Word 文档信息
    source_file: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="源模板文件名")
    working_file: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="工作副本文件名")
    paragraph_start: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="起始段落索引")
    paragraph_end: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="结束段落索引")

    # 状态
    has_content: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", comment="是否有内容")
    has_assets: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", comment="是否有素材")
...
class ChapterAsset(BaseModel):
    ...
    original_filename: Mapped[str] = mapped_column(String(300), nullable=False, comment="原始文件名")
    file_path: Mapped[str] = mapped_column(String(500), nullable=False, comment="文件存储路径")
    file_type: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="文件类型: pdf/docx/xlsx/txt")
    file_size: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="文件大小(字节)")
    uploaded_at = mapped_column(
        DateTime(timezone=True),
        nullable=False,
...
class OcrExtractionTask(BaseModel):
    ...
    chapter = relationship("DossierChapter", foreign_keys=[chapter_id])

...
class DossierRepository:
    ...
    async def get_asset_usage(self, chapter_id: UUID, asset_id: UUID) -> "ChapterAssetUsage | None":
        """获取单个素材使用记录"""
        from .field_models import ChapterAssetUsage
    ...
    async def update_asset_usage(self, usage_id: UUID, is_selected: bool) -> "ChapterAssetUsage | None":
        """更新素材使用状态"""
        from .field_models import ChapterAssetUsage
    ...
    async def get_ancestor_chapter_ids(self, chapter_id: UUID) -> list[UUID]:
        ...
        while True:
            stmt = select(DossierChapter.parent_id).where(DossierChapter.id == current_id)
            result = await self.db.execute(stmt)

...
from app.modules.registration.dossier_writer.models import ProductDossier, DossierChapter, ChapterAsset
...
async def run_t9():
    ...
    async for db in get_db():
        ...
        template = await service.save_template_file(dossier.id, "s6_template.docx", content)
        ...
        # Get S6 chapter
        s6_stmt = select(DossierChapter).where(and_(DossierChapter.product_dossier_id == dossier.id, DossierChapter.chapter_code == "3.2.S.6"))
        ...
        mock_fields = [{"field_name": "包装系统描述", "value": "T-9 Real Write Value", "confidence": 0.95, "field_type": "text", "field_mapping_id": None}]
        ...
        asset = ChapterAsset(chapter_id=s6_chapter.id, original_filename="test_split.png", file_path=dummy_png, file_type="png")
        db.add(asset)
        await db.commit()

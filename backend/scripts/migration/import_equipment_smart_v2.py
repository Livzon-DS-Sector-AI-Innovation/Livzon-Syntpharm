        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if key in active_equipments:
                        # 已有活跃设备，跳过
                        skipped_count += 1
                        
                        # 如果还有已删除的重复记录，删除它
                        if key in deleted_equipments:
                            deleted_id = deleted_equipments[key]
                            await db.execute(
                                delete(Equipment).where(Equipment.id == deleted_id)
                            )
                            duplicate_removed += 1
                            
                    elif key in deleted_equipments:
                        # 只有已删除的设备，恢复它
                        deleted_id = deleted_equipments[key]
                        stmt = (
                            update(Equipment)
                            .where(Equipment.id == deleted_id)
                            .values(
                                is_deleted=False,
                                name=name,
                        ...
                        # 移动到活跃索引
                        active_equipments[key] = deleted_id
                        del deleted_equipments[key]
                        ...
                        # 添加到索引
                        active_equipments[key] = new_equip.id
            ...
            print(f"   删除重复的已删除记录: {duplicate_removed} 条")

    ...
    async with async_session_factory() as db:
        try:
            ...
            result = await db.execute(
                select(Equipment.category_description)
                .where(Equipment.category_description.isnot(None))
                .distinct()
            )
            ...
            all_categories_result = await db.execute(
                select(EquipmentCategory.id, EquipmentCategory.name)
            )
            ...
            # 先清空现有的关联（避免重复）
            await db.execute(
                update(EquipmentCategoryLink).values(is_deleted=True)
            )
            await db.commit()
            ...
            total_links = await db.execute(
                select(EquipmentCategoryLink).where(~EquipmentCategoryLink.is_deleted)
            )

...
def get_standard_dept(raw_dept, valid_depts):
    ...
    if raw in DEPT_MAPPING: return DEPT_MAPPING[raw]
...
async def main():
    ...
    async with async_session_factory() as db:
        try:
            ...
            print("🧹 正在清理已删除的重复记录...")
            await db.execute(update(Equipment).where(Equipment.is_deleted == True).values(is_deleted=True)) # 确保状态一致
            ...
            updated, inserted, migrated, skipped = 0, 0, 0, 0
            processed_ids = set()
            ...
            for _, row in df.iterrows():
                ...
                if target_equip:
                    # A. 完全匹配：直接更新
                    stmt = update(Equipment).where(Equipment.id == target_equip.id).values(
                        ...
                        status='在用', model=str(row['型号']).strip() if pd.notna(row['型号']) else None,
                    ...
                    stmt = update(Equipment).where(Equipment.id == old_equip.id).values(
                        name=str(row['设备名称']).strip(),
                        department_id=dept_id, location_id=loc_id, location_text=loc_text,
                        current_cost=float(row['当前成本']) if pd.notna(row['当前成本']) else None,
                        book_value=float(row['帐面净值']) if pd.notna(row['帐面净值']) else None,
                    ...
                    # C. 新增
                    new_e = Equipment(
                        asset_no=asset_no, name=str(row['设备名称']).strip(),
                        department_id=dept_id, location_id=loc_id, location_text=loc_text,
                        current_cost=float(row['当前成本']) if pd.notna(row['当前成本']) else None,
                        book_value=float(row['帐面净值']) if pd.notna(row['帐面净值']) else None,
                        status='在用', is_deleted=False
                    )
                    db.add(new_e)
                    inserted += 1

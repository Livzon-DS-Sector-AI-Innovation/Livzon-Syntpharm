...
async def main():
    async with async_session_factory() as db:
        try:
            ...
            # Step 6: 验证结果
            print("\n📊 步骤 6: 验证结果...")
            with_location = await db.execute(select(Equipment).where(Equipment.location_id.isnot(None)))
            with_location_count = len(with_location.scalars().all())

            without_location = await db.execute(select(Equipment).where(Equipment.location_id.is_(None)))

...
async def main():
    async with async_session_factory() as db:
        try:
            ...
            # Step 4: 验证结果
            print("\n📊 步骤 4: 验证结果...")
            with_dept = await db.execute(select(Equipment).where(Equipment.department_id.isnot(None)))
            with_dept_count = len(with_dept.scalars().all())

            without_dept = await db.execute(select(Equipment).where(Equipment.department_id.is_(None)))

...
export function ComplianceAlert({ 
  ...
  if (q3cClass1Count > 0) {
    alerts.push(
      <Alert
        key="q3c-class1"
        message={`检出 ${q3cClass1Count} 种 Class 1 溶剂`}
        description={
          <span>
  ...
  if (q3cUnknownCount > 0) {
    alerts.push(
      <Alert
        key="q3c-unknown"
        message={`检出 ${q3cUnknownCount} 种未列入 ICH Q3C 的溶剂`}
  ...
  if (q3dClass1Found.length > 0) {
    alerts.push(
      <Alert
        key="q3d-class1"
        message={`文中检出 ${q3dClass1Found.length} 种 Class 1 元素：${q3dClass1Found.join('、')}`}
        description={
          <span>
  ...
  if (q3dClass2BIntentionalCount > 0) {
    alerts.push(
      <Alert
        key="q3d-class2b"
        message={`检出 ${q3dClass2BIntentionalCount} 种有意添加的 Class 2B 元素`}

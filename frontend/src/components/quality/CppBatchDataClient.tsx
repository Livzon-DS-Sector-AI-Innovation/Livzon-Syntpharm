...
import { CpvImportDrawer } from "@/components/quality/CpvImportDrawer"

interface Props {
  productId: string
  initialProduct: CpvProduct
  initialParameters: CpvParameter[]
}
...
export function CppBatchDataClient({ productId, initialProduct, initialParameters }: Props) {
  ...
  async function handleExport() {
    try {
      ...
      const productName = initialProduct?.name || "未知产品";
  ...
  if (loading && batches.length === 0) return <div className="p-6 text-center text-gray-500">加载中...</div>

...
import { fetchCpvStatistics, fetchCpvTrend } from "@/lib/api/client/quality-cpv"
import { CpvProduct, CpvParameter, CpvStatistics, CpvTrendResponse } from "@/types/quality-cpv"
import { CpvTrendChart } from "@/components/quality/CpvTrendChart"

interface Props {
  productId: string
  initialProduct: CpvProduct
  initialCppParams: CpvParameter[]
  initialCqaParams: CpvParameter[]
}

export function CpvProductDetailClient({ productId, initialProduct, initialCppParams, initialCqaParams }: Props) {
  const [cppParams] = useState<CpvParameter[]>(initialCppParams)
  const [cqaParams] = useState<CpvParameter[]>(initialCqaParams)
  const [selectedType, setSelectedType] = useState<"CPP" | "CQA">("CPP")
  const [selectedParamId, setSelectedParamId] = useState<string>(initialCppParams[0]?.id || "")
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="text-sm text-gray-500">
        ...
        <span className="text-gray-900">{initialProduct.name}</span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
          <div><span className="text-gray-500">产品名称</span><div className="font-medium text-gray-900 mt-1">{initialProduct.name}</div></div>
          <div><span className="text-gray-500">规格</span><div className="font-medium text-gray-900 mt-1">{initialProduct.specification || "-"}</div></div>
          <div><span className="text-gray-500">工艺版本</span><div className="font-medium text-gray-900 mt-1">{initialProduct.process_version || "-"}</div></div>
          <div><span className="text-gray-500">状态</span><div className="mt-1"><span className={`px-2 py-1 text-xs rounded-full font-semibold ${initialProduct.status === "active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{initialProduct.status === "active" ? "启用" : "停用"}</span></div></div>
          <div><span className="text-gray-500">创建时间</span><div className="font-medium text-gray-900 mt-1">{new Date(initialProduct.created_at).toLocaleDateString("zh-CN")}</div></div>
        </div>
      </div>

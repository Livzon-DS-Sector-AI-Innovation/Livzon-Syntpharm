...
import type { Personnel, EquipmentCategory } from '@/types/equipment'
import { fetchPersonnelList, fetchCategoriesClient } from '@/lib/api/client/equipment'
...
export function MaintenancePlanDrawer({ equipments, onRefresh }: MaintenancePlanDrawerProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchPersonnelList({}).then(r => setPersonnel(r.filter(p => p.is_active))),
            ...
            {planMode === 'equipment' ? (
              <Form.Item name="equipment_id" label="关联设备" rules={[{ required: true, message: '请选择设备' }]}>
                <Select placeholder="选择设备" showSearch optionFilterProp="label"
                  options={equipments.map((eq) => ({ label: `${eq.asset_no} - ${eq.name}`, value: eq.id }))}
                />
              </Form.Item>
            ) : (

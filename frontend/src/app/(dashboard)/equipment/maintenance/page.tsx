...
export default async function MaintenancePageWrapper() {
  ...
  try {
    ...
    equipments = Array.isArray(result[0]?.data) ? result[0].data : (result[0]?.items ?? [])
    workOrders = result[1]?.items ?? (Array.isArray(result[1]?.data) ? result[1].data : [])
    workOrderTotal = result[1]?.total ?? workOrders.length
    ...
    calibrationPlans = result[6]?.items ?? (Array.isArray(result[6]?.data) ? result[6].data : [])
    calibrationPlanTotal = result[6]?.total ?? calibrationPlans.length
    calibrationRecords = result[7]?.items ?? (Array.isArray(result[7]?.data) ? result[7].data : [])
    calibrationRecordTotal = result[7]?.total ?? calibrationRecords.length
    maintenancePlans = result[8]?.items ?? (Array.isArray(result[8]?.data) ? result[8].data : [])
    maintenancePlanTotal = result[8]?.total ?? maintenancePlans.length
  } catch (error) {

...
export default async function RawMaterialPage({ searchParams }: PageProps) {
  ...
  try {
    tables = await fetchWarehouseFeishuTablesByParams({
      business_domain: 'finished_product',
      enabled: true,
    })
  } catch (error) {
    loadError = error instanceof Error ? error.message : '成品飞书数据加载失败'
    console.warn('成品飞书表目录加载失败:', error)
  }

  if (!loadError && !params.table_id && tables[0]?.id) {
    redirect(`/warehouse/raw-material?table_id=${tables[0].id}`)
  }

  if (!loadError) {
    try {
      data = await fetchWarehouseFeishuDomainRecords('finished_product', {
  ...
  return (
    <FeishuDomainMonitorClient
      ...
      businessDomain="finished_product"
      title="成品"
      description="展示成品多维表格中已启用数据表的本地同步快照"
      tables={tables}
      data={data}
      error={loadError}

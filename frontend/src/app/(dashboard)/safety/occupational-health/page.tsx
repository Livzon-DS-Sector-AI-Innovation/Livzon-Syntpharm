...
export default function OccupationalHealthPage() {
  ...
  const handleMonitorSave = async () => {
    try {
      ...
      const formattedValues = {
        ...values,
        detection_date: values.detection_date ? values.detection_date.toISOString() : undefined,
      }
      setMonitorSaving(true)
      let res
      if (editingMonitor) {
        res = await updateOhHazardMonitor(editingMonitor.id, formattedValues)
      } else {
        res = await createOhHazardMonitor(formattedValues)
      }
      if (res.code === 200) {
  ...
  const handleExamSave = async () => {
    try {
      ...
      let res
      if (editingExam) {
        res = await updateOhHealthExam(editingExam.id, formattedValues)
      } else {
        res = await createOhHealthExam(formattedValues)
      }
      if (res.code === 200) {
                ...
                children: (
                  <AbnormalityTab
                    ...
                    onAdd={async (data) => {
                      ...
                      if (res.code === 200) { message.success('已添加'); openMonitorDetail(selectedMonitor) }
                    }}
                    onUpdateStatus={async (index, status) => {
                      const res = await updateMonitorAbnormalityStatus(selectedMonitor.id, index, status)
                      if (res.code === 200) { message.success('已更新'); openMonitorDetail(selectedMonitor) }
                    }}
                  />
                ),
              },
                ...
                children: (
                  <AbnormalityTab
                    ...
                    onAdd={async (data) => {
                      ...
                      if (res.code === 200) { message.success('已添加'); openExamDetail(selectedExam) }
                    }}
                    onUpdateStatus={async (index, status) => {
                      const res = await updateExamAbnormalityStatus(selectedExam.id, index, status)
                      if (res.code === 200) { message.success('已更新'); openExamDetail(selectedExam) }
                    }}
                  />
                ),
              },

...
// ==================== 巡检模板 ====================
export async function fetchInspectionTemplatesClient(params: { is_active?: boolean; keyword?: string; page?: number; page_size?: number } = {}): Promise<InspectionTemplateListResponse> {
  ...
  return apiFetchPaginated(`${API_BASE}/equipment/maintenance/inspection-templates/${qs ? `?${qs}` : ''}`)
}
...
export async function fetchEquipmentsClient(params: { page?: number; page_size?: number; keyword?: string } = {}): Promise<{ items: any[]; total: number }> {
  ...
  if (params.page_size) searchParams.append('page_size', params.page_size.toString())
  const qs = searchParams.toString()
  return apiFetchPaginated(`${API_BASE}/equipment/equipments${qs ? `?${qs}` : ''}`)
}
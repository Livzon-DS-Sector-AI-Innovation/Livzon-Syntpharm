...
export async function fetchRoles(): Promise<EquipmentRole[]> {
  return apiGet(`${API_BASE}/equipment/personnel/roles?page_size=500`)
}

export async function fetchRole(id: string): Promise<EquipmentRole> {
  return apiGet(`${API_BASE}/equipment/personnel/roles/${id}`)
}
...
export async function fetchPersonnelList(params?: {
  ...
  return apiFetchPaginated(`${API_BASE}/equipment/personnel${qs ? `?${qs}` : ''}`)
}

export async function fetchPersonnel(id: string): Promise<Personnel> {
  return apiGet(`${API_BASE}/equipment/personnel/${id}`)
}
...
export async function fetchCandidates(
  ...
  return apiGet(`${API_BASE}/equipment/personnel/candidates?${params.toString()}`)
}
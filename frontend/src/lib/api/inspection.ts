...
const INSPECTION_BASE = `/api/v1/equipment/inspection`
...
export async function fetchInspectionTemplatesClient(params: { is_active?: boolean; keyword?: string; page?: number; page_size?: number } = {}) {
  ...
  const url = qs ? `/api/v1/equipment/maintenance/inspection-templates/?${qs}` : `/api/v1/equipment/maintenance/inspection-templates/`
  const response = await fetch(url, { headers: { 'Content-Type': 'application/json' } })
  if (!response.ok) throw new Error(`请求失败: ${response.status}`)
  const result = await response.json()
  return { items: result.data || [], total: result.meta?.total || 0, page: result.meta?.page || 1, page_size: result.meta?.page_size || 20 }
...
export async function fetchInspectionTemplateByIdClient(id: string) {
  const response = await fetch(`/api/v1/equipment/maintenance/inspection-templates/${id}`, { headers: { 'Content-Type': 'application/json' } })
  if (!response.ok) throw new Error(`请求失败: ${response.status}`)
  const result = await response.json()
  return result.data
}
...
export async function fetchEquipmentsClient(params: { page?: number; page_size?: number; keyword?: string } = {}) {
  ...
  if (params.page_size) searchParams.append('page_size', params.page_size.toString())
  const qs = searchParams.toString()
  const url = qs ? `/api/v1/equipment/equipments?${qs}` : `/api/v1/equipment/equipments`
  const response = await fetch(url, { headers: { 'Content-Type': 'application/json' } })
  if (!response.ok) throw new Error(`请求失败: ${response.status}`)
  const result = await response.json()
  return { items: result.data || [], total: result.meta?.total || 0, page: result.meta?.page || 1, page_size: result.meta?.page_size || 20 }
...
export async function fetchRouteSchedules(routeId?: string): Promise<any[]> {
  const url = routeId ? `/api/v1/equipment/inspection/routes/${routeId}/schedules` : `/api/v1/equipment/inspection/routes/schedules`

...
export async function getUsers(params?: { keyword?: string; status?: string }, token?: string) {
  ...
  return apiFetch(`${API_BASE_URL}/api/v1/identity/users${query ? `?${query}` : ''}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
  })
}

export async function createUser(data: any, token?: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/identity/users`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
...
export async function updateUser(id: string, data: any, token?: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/identity/users/${id}`, {
    method: 'PUT',
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    body: JSON.stringify(data),
  })
...
export async function resetUserPassword(id: string, data: any, token?: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/identity/users/${id}/reset-password`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    body: JSON.stringify(data),
  })

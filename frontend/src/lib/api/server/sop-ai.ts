...
export async function getConfigs() {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/config`)
}

export async function getConfig(configKey: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/config/${configKey}`)
}
...
export async function singleCheck(data: any) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/check/single`, {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function batchCheck(data: any) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/check/batch`, {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function getCheckRecords(params: Record<string, unknown> = {}) {
  ...
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/records${query}`)
}

export async function getCheckRecordDetail(id: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/records/${id}`)
}

export async function exportCheckReport(id: string, format: string = 'excel', includeProblems: boolean = true) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/export/${id}?format=${format}&include_problems=${includeProblems}`)
}

export async function handleProblem(problemId: string, data: any) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/problems/${problemId}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function getScheduledJobs() {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/jobs`)
}
...
export async function deleteScheduledJob(jobId: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/jobs/${jobId}`, {
    method: 'DELETE',
  })
}
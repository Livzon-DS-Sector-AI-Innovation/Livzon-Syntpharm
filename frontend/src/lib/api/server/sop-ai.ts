...
export async function getConfig(configKey: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/quality/sop-ai/config/${configKey}`)
}
...
export async function exportCheckReport(id: string, format: string = 'excel', includeProblems: boolean = true) {
  return apiFetch(`${API_BASE_URL}/api/v1/quality/sop-ai/export/${id}?format=${format}&include_problems=${includeProblems}`)
}
...
export async function getScheduledJobs() {
  return apiFetch(`${API_BASE_URL}/api/v1/quality/sop-ai/jobs`)
}
...
export async function deleteScheduledJob(jobId: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/quality/sop-ai/jobs/${jobId}`, {
    method: 'DELETE',
  })
}
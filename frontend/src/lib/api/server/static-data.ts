...
export async function getDictEquipmentCategory() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/equipment-category`)
}

export async function getDictEquipmentStatus() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/equipment-status`)
}

export async function getDictVerifyStatus() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/verify-status`)
}

export async function getDictChromColumnStatus() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/chrom-column-status`)
}

export async function getDictMediumType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/medium-type`)
}

export async function getDictReagentPurity() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/reagent-purity`)
}

export async function getDictDangerType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/danger-type`)
}

export async function getDictStdType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/std-type`)
}

export async function getDictMaterialType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/material-type`)
}

export async function getDictStandardSource() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/standard-source`)
}

export async function getDictLimitType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/limit-type`)
}

export async function getDictTestItemCategory() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/test-item-category`)
}

export async function getDictUnitType() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/unit-type`)
}

export async function getDictLab() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/dict/lab`)
}
...
export async function listStorageCondition(params: Record<string, unknown> = {}) {
  ...
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/storage-condition?${qs}`)
}

export async function getStorageConditionOptions() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/storage-condition/options`)
}
...
export async function listUnit(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/unit?${qs}`)
}

export async function getUnitOptions() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/unit/options`)
}
...
export async function listTestItem(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/test-item?${qs}`)
}

export async function getTestItemOptions() {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/test-item/options`)
}
...
export async function listEquipment(params: Record<string, unknown> = {}) {
  ...
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/equipment?${qs}`)
}
...
export async function listChromColumn(params: Record<string, unknown> = {}) {
  ...
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/chrom-column?${qs}`)
}
...
export async function incrementChromColumnUsage(id: number) {
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/chrom-column/${id}/increment-usage`, {
    method: 'POST',
  })
}
...
export async function listMedium(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/medium?${qs}`)
}
...
export async function listReagent(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/reagent?${qs}`)
}
...
export async function listStandardMaterial(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/standard-material?${qs}`)
}
...
export async function listMaterialStandard(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/material-standard?${qs}`)
}
...
export async function listProductStandard(params: Record<string, unknown> = {}) {
  ...
  if (params.status !== undefined) qs.set('status', String(params.status))
  return apiFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/product-standard?${qs}`)
}
...
export async function uploadFile(formData: FormData) {
  return uploadFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/upload`, formData)
}

export async function getDownloadUrl(filename: string) {
  return `${getApiBaseUrl()}/api/v1${PREFIX}/download/${encodeURIComponent(filename)}`
}

// ===== Excel 导入导出 =====

export async function downloadExcelTemplate(moduleType: string) {
  return blobFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/excel/template/${moduleType}`)
}

export async function importExcel(moduleType: string, formData: FormData) {
  return uploadFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/excel/import/${moduleType}`, formData)
}
...
export async function exportExcel(params: Record<string, unknown> = {}) {
  ...
  if (params.start_date) qs.set('start_date', String(params.start_date))
  if (params.end_date) qs.set('end_date', String(params.end_date))
  return blobFetch(`${getApiBaseUrl()}/api/v1${PREFIX}/excel/export/${params.module_type}?${qs}`)
}
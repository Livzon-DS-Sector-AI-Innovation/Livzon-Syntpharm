...
export async function getCheckProgressApi(taskId: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/records/${taskId}`)
}
...
export async function exportCheckReportApi(id: string, format: 'pdf' | 'excel' = 'pdf') {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/export/${id}?format=${format}`)
}
...
export async function updateCheckConfigApi(key: string, value: string, operator?: string) {
  return apiFetch(`${API_BASE_URL}/api/v1/sop-ai/config/${key}`, {
    method: 'PUT',
    body: JSON.stringify({
      config_value: value,
      operator,

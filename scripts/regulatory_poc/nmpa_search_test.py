...
def main():
    ...
    for inp in page_info.get("inputs", []):
        print(f"   📝 input: type={inp['type']} name={inp['name']} id={inp['id']} placeholder={inp.get('placeholder','')}")
    ...
    # 先检查是否有搜索输入框
    search_input = page.query_selector('input[type="text"], input[name*="keyword"], input[name*="search"], input.search-input, #keyword, .search-txt input')
    if search_input:
        print(f"   ✅ 找到搜索框")
        ...
        # 找搜索按钮
        search_btn = page.query_selector('button[type="submit"], .search-btn, input[type="submit"], button:has-text("搜索"), button:has-text("查询"), .search-button')
        if search_btn:
            print(f"   ✅ 找到搜索按钮，点击...")
            search_btn.click()
        else:
